...
static void snap_preview_make_clickthrough(server_t *s) {
  const xcb_query_extension_reply_t *ext =
      xcb_get_extension_data(s->conn, &xcb_shape_id);
  if (!ext || !ext->present)
    return;

  // empty input region so pointer events pass through
  xcb_shape_rectangles(s->conn, XCB_SHAPE_SO_SET, XCB_SHAPE_SK_INPUT,
                       XCB_CLIP_ORDERING_UNSORTED, s->snap_preview_win, 0, 0, 0,
                       NULL);
}
...
void snap_preview_init(server_t *s) {
  ...
  xcb_create_window(s->conn, s->root_depth, s->snap_preview_win, s->root, 0, 0,
                    1, 1, s->snap_preview_border_px,
...
void snap_preview_apply(server_t *s, const rect_t *rect, bool show) {
  ...
  if (s->root_depth != 32)
    border &= 0x00FFFFFFu;
  xcb_change_window_attributes(s->conn, s->snap_preview_win,
                               XCB_CW_BORDER_PIXEL, &border);
