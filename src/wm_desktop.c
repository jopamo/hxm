...
static bool wm_client_should_be_visible_now(const server_t *s,
                                            const client_hot_t *hot) {
  if (!s || !hot)
    return false;
    ...
    for (size_t i = 0; i < s->active_clients.length; i++) {
      ...
      if (hot && hot->show_desktop_hidden) {
        ...
        if (hot->state == STATE_UNMAPPED &&
            wm_client_should_be_visible_now(s, hot)) {
...
void wm_publish_desktop_props(server_t *s) {
  ...
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root,
                      atoms._NET_NUMBER_OF_DESKTOPS, XCB_ATOM_CARDINAL, 32, 1,
                      &s->desktop_count);
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root,
  ...
  if (vroots) {
    ...
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root,
                        atoms._NET_VIRTUAL_ROOTS, XCB_ATOM_WINDOW, 32,
                        s->desktop_count, vroots);
    free(vroots);
  }

  bool publish_names =
      s->config.desktop_names && s->config.desktop_names_count > 0;
  if (!publish_names) {
    xcb_get_property_cookie_t ck = xcb_get_property(
        conn, 0, root, atoms._NET_DESKTOP_NAMES, atoms.UTF8_STRING, 0, 1024);
  ...
  if (publish_names) {
    ...
    if (buf) {
      ...
      xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root,
                          atoms._NET_DESKTOP_NAMES, atoms.UTF8_STRING, 8,
    ...
    for (uint32_t m = 0; m < m_count; m++) {
      ...
      if (cold->strut.left > 0) {
        ...
        if (cold->strut_partial_active) {
          overlap = (cold->strut.left_start_y < (uint32_t)mb &&
                     cold->strut.left_end_y > (uint32_t)mt);
        }
        if (overlap) {
      ...
      if (cold->strut.right > 0) {
        ...
        bool overlap = true;
        if (cold->strut_partial_active) {
          overlap = (cold->strut.right_start_y < (uint32_t)mb &&
                     cold->strut.right_end_y > (uint32_t)mt);
        }
        if (overlap) {
          if (r_edge < mr)
      ...
      if (cold->strut.top > 0) {
        bool overlap = true;
        if (cold->strut_partial_active) {
          overlap = (cold->strut.top_start_x < (uint32_t)mr &&
                     cold->strut.top_end_x > (uint32_t)ml);
        }
        if (overlap) {
      ...
      if (cold->strut.bottom > 0) {
        ...
        bool overlap = true;
        if (cold->strut_partial_active) {
          overlap = (cold->strut.bottom_start_x < (uint32_t)mr &&
                     cold->strut.bottom_end_x > (uint32_t)ml);
        }
        if (overlap) {
          if (b_edge < mb)
  ...
  if (!focused_visible) {
    ...
    for (list_node_t *node = s->focus_history.next; node != &s->focus_history;
      ...
      if (c->state == STATE_MAPPED &&
          (c->desktop == (int32_t)new_desktop || c->sticky)) {
        new_focus = c->self;
        break;
      }
    }
...
void wm_client_move_to_workspace(server_t *s, handle_t h, uint32_t desktop,
                                 bool follow) {
  client_hot_t *c = server_chot(s, h);
  if (!c)
    return;
  ...
  if (c->sticky &&
      (c->type == WINDOW_TYPE_DOCK || c->type == WINDOW_TYPE_DESKTOP) &&
      desktop != 0xFFFFFFFF) {
    desktop = 0xFFFFFFFF;
  }
  ...
  LOG_INFO("Moving client %u to desktop %d (follow=%d)", c->xid, new_desk,
           follow);
  ...
  if (follow && !c->sticky) {
    ...
    if (!visible && s->focused_client == h) {
      ...
      for (list_node_t *node = s->focus_history.next; node != &s->focus_history;
        ...
        client_hot_t *hc =
            (client_hot_t *)((char *)node - offsetof(client_hot_t, focus_node));
        if (hc->state == STATE_MAPPED &&
            (hc->desktop == (int32_t)s->current_desktop || hc->sticky)) {
          new_focus = hc->self;
          break;
        }
      }
