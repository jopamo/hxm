...
void wm_send_sync_request(server_t *s, const client_hot_t *hot, uint64_t value,
                          uint32_t time) {
  ...
  ev.data.data32[4] = 0;
  xcb_send_event(s->conn, 0, hot->xid, XCB_EVENT_MASK_NO_EVENT,
                 (const char *)&ev);
}
...
static void wm_clamp_rect_to_bounds(rect_t *r, const rect_t *bounds) {
  ...
  if (x + (int32_t)r->w > bwmax)
    x = bwmax - (int32_t)r->w;
  if (y + (int32_t)r->h > bhmax)
    y = bhmax - (int32_t)r->h;
...
void wm_send_synthetic_configure(server_t *s, handle_t h) {
  ...
  if (hot->last_synthetic_geom.x == ev->x &&
  ...
  TRACE_LOG("synthetic_configure xid=%u x=%d y=%d w=%u h=%u", hot->xid, ev->x,
...
void wm_publish_workarea(server_t *s, const rect_t *wa) {
  ...
  if (changed) {
    ...
    if (wa_vals) {
      ...
      xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, s->root,
                          atoms._NET_WORKAREA, XCB_ATOM_CARDINAL, 32, n * 4,
                          wa_vals);
      free(wa_vals);
    }
  }
  ...
  for (size_t i = 0; i < s->active_clients.length; i++) {
    ...
    if (hot->state == STATE_UNMANAGING || hot->state == STATE_DESTROYED)
      continue;
...
static uint32_t wm_build_client_list_stacking(server_t *s, xcb_window_t *out,
                                              uint32_t cap) {
  if (!s || !out || !cap)
    return 0;
...
static uint32_t wm_build_client_list(server_t *s, xcb_window_t *out,
                                     uint32_t cap) {
  if (!s || !out || !cap)
    return 0;
...
bool wm_flush_dirty(server_t *s, uint64_t now) {
  ...
  for (size_t i = 0; i < s->active_clients.length;) {
    ...
    // Safe advance: only increment if the element at 'i' hasn't changed (wasn't
    // swapped out)
  ...
  // Preview window for snap-to-edge
  {
    ...
    if (should_show)
      preview_rect = preview_hot->snap_preview_frame_rect;
    snap_preview_apply(s, should_show ? &preview_rect : NULL, should_show);

    if (should_show || was_mapped != s->snap_preview_mapped)
      flushed = true;
  }
  ...
  if (s->root_dirty & ROOT_DIRTY_VISIBILITY) {
    ...
    for (size_t i = 0; i < s->active_clients.length;) {
      ...
      if (visible) {
        ...
        xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, c->xid,
                            atoms.WM_STATE, atoms.WM_STATE, 32, 2, state_vals);
      } else {
        if (c->ignore_unmap < UINT8_MAX)
          c->ignore_unmap++;
  ...
  for (size_t i = 0; i < s->active_clients.length;) {
    ...
    bool dirty_interesting =
        (hot->dirty & (DIRTY_GEOM | DIRTY_STACK | DIRTY_STATE));
    if (dirty_changed || dirty_interesting) {
      TRACE_LOG("flush_dirty h=%lx xid=%u dirty=0x%x state=%d", h, hot->xid,
                hot->dirty, hot->state);
    ...
    if (hot->dirty & DIRTY_GEOM) {
      bool interactive = ((s->interaction_mode == INTERACTION_RESIZE ||
                           s->interaction_mode == INTERACTION_MOVE) &&
                          s->interaction_window == hot->frame);

      if (interactive) {
        uint64_t interval = 16666666; // ~16ms for 60Hz
        if (s->last_interaction_flush > 0 &&
            (now - s->last_interaction_flush) < interval) {
      ...
      bool interactive_resize = (s->interaction_mode == INTERACTION_RESIZE &&
                                 s->interaction_window == hot->frame);
      if (interactive_resize && hot->sync_enabled &&
          hot->sync_counter != XCB_NONE) {
      ...
      TRACE_LOG(
          "apply_geom: frame(%dx%d+%d+%d) extents_set=%d -> client(%dx%d)",
          frame_w, frame_h, frame_x, frame_y, hot->gtk_frame_extents_set,
          client_w, client_h);

      bool geom_changed = (hot->server.x != (int16_t)frame_x ||
      ...
      if (geom_changed) {
        if (!interactive_resize && hot->sync_enabled &&
            hot->sync_counter != XCB_NONE) {
          if (hot->server.w != (uint16_t)client_w ||
              hot->server.h != (uint16_t)client_h) {
            uint64_t sync_value = ++hot->sync_value;
        ...
        LOG_DEBUG("Flushed DIRTY_GEOM for %lx: Frame Global(%d,%d) Client "
                  "Local(%d,%d) %dx%d",
                  h, frame_x, frame_y, local_x, local_y, client_w, client_h);
        flushed = true;
      } else {
  ...
  end_dirty_geom:
    ...
    if (hot->dirty & DIRTY_TITLE) {
      flushed = true;
      uint32_t c = xcb_get_property(s->conn, 0, hot->xid, atoms._NET_WM_NAME,
                                    atoms.UTF8_STRING, 0, 1024)
                       .sequence;
      cookie_jar_push(&s->cookie_jar, c, COOKIE_GET_PROPERTY, h,
                      ((uint64_t)hot->xid << 32) | atoms._NET_WM_NAME,
                      s->txn_id, wm_handle_reply);

      c = xcb_get_property(s->conn, 0, hot->xid, atoms.WM_NAME, XCB_ATOM_STRING,
                           0, 1024)
              .sequence;
      cookie_jar_push(&s->cookie_jar, c, COOKIE_GET_PROPERTY, h,
                      ((uint64_t)hot->xid << 32) | atoms.WM_NAME, s->txn_id,
                      wm_handle_reply);
    ...
    if (hot->dirty & DIRTY_HINTS) {
      flushed = true;
      uint32_t c = xcb_get_property(s->conn, 0, hot->xid, atoms.WM_NORMAL_HINTS,
                                    XCB_ATOM_WM_SIZE_HINTS, 0, 32)
                       .sequence;
      cookie_jar_push(&s->cookie_jar, c, COOKIE_GET_PROPERTY, h,
                      ((uint64_t)hot->xid << 32) | atoms.WM_NORMAL_HINTS,
                      s->txn_id, wm_handle_reply);

      c = xcb_get_property(s->conn, 0, hot->xid, atoms.WM_HINTS, atoms.WM_HINTS,
                           0, 32)
              .sequence;
      cookie_jar_push(&s->cookie_jar, c, COOKIE_GET_PROPERTY, h,
                      ((uint64_t)hot->xid << 32) | atoms.WM_HINTS, s->txn_id,
                      wm_handle_reply);

      c = xcb_get_property(s->conn, 0, hot->xid, atoms.WM_COLORMAP_WINDOWS,
                           XCB_ATOM_WINDOW, 0, 64)
              .sequence;
      cookie_jar_push(&s->cookie_jar, c, COOKIE_GET_PROPERTY, h,
                      ((uint64_t)hot->xid << 32) | atoms.WM_COLORMAP_WINDOWS,
                      s->txn_id, wm_handle_reply);

      c = xcb_get_property(s->conn, 0, hot->xid, atoms._MOTIF_WM_HINTS,
                           XCB_ATOM_ANY, 0, 5)
              .sequence;
      cookie_jar_push(&s->cookie_jar, c, COOKIE_GET_PROPERTY, h,
                      ((uint64_t)hot->xid << 32) | atoms._MOTIF_WM_HINTS,
                      s->txn_id, wm_handle_reply);

      c = xcb_get_property(s->conn, 0, hot->xid, atoms._GTK_FRAME_EXTENTS,
                           XCB_ATOM_CARDINAL, 0, 4)
              .sequence;
      cookie_jar_push(&s->cookie_jar, c, COOKIE_GET_PROPERTY, h,
                      ((uint64_t)hot->xid << 32) | atoms._GTK_FRAME_EXTENTS,
                      s->txn_id, wm_handle_reply);
    ...
    if (hot->dirty & DIRTY_STRUT) {
      ...
      uint32_t c =
          xcb_get_property(s->conn, 0, hot->xid, atoms._NET_WM_STRUT_PARTIAL,
              ...
              .sequence;
      cookie_jar_push(&s->cookie_jar, c, COOKIE_GET_PROPERTY, h,
                      ((uint64_t)hot->xid << 32) | atoms._NET_WM_STRUT_PARTIAL,
                      s->txn_id, wm_handle_reply);

      c = xcb_get_property(s->conn, 0, hot->xid, atoms._NET_WM_STRUT,
                           XCB_ATOM_CARDINAL, 0, 4)
              .sequence;
      cookie_jar_push(&s->cookie_jar, c, COOKIE_GET_PROPERTY, h,
                      ((uint64_t)hot->xid << 32) | atoms._NET_WM_STRUT,
                      s->txn_id, wm_handle_reply);
    ...
    if (hot->dirty & DIRTY_OPACITY) {
      flushed = true;
      uint32_t c =
          xcb_get_property(s->conn, 0, hot->xid, atoms._NET_WM_WINDOW_OPACITY,
                           XCB_ATOM_CARDINAL, 0, 1)
              .sequence;
      cookie_jar_push(&s->cookie_jar, c, COOKIE_GET_PROPERTY, h,
                      ((uint64_t)hot->xid << 32) | atoms._NET_WM_WINDOW_OPACITY,
                      s->txn_id, wm_handle_reply);
    ...
    if (hot->frame_damage.valid ||
        (hot->dirty &
         (DIRTY_FRAME_ALL | DIRTY_FRAME_TITLE | DIRTY_FRAME_BUTTONS |
    ...
    if (hot->dirty & DIRTY_STACK) {
      flushed = true;
      TRACE_LOG("flush_dirty stack h=%lx layer=%d stack_layer=%d", h,
                hot->layer, hot->stacking_layer);
    ...
    if (hot->dirty & DIRTY_STATE) {
      flushed = true;
      TRACE_LOG("flush_dirty state h=%lx layer=%d above=%d below=%d sticky=%d "
                "max=%d/%d focused=%d",
                h, hot->layer, hot->state_above, hot->state_below, hot->sticky,
                hot->maximized_horz, hot->maximized_vert,
                (hot->flags & CLIENT_FLAG_FOCUSED) != 0);
      ...
      if (hot->flags & CLIENT_FLAG_URGENT)
        state_atoms[count++] = atoms._NET_WM_STATE_DEMANDS_ATTENTION;
      if (hot->sticky)
        state_atoms[count++] = atoms._NET_WM_STATE_STICKY;
      if (hot->skip_taskbar)
        state_atoms[count++] = atoms._NET_WM_STATE_SKIP_TASKBAR;
      if (hot->skip_pager)
        state_atoms[count++] = atoms._NET_WM_STATE_SKIP_PAGER;
      if (hot->maximized_horz)
        state_atoms[count++] = atoms._NET_WM_STATE_MAXIMIZED_HORZ;
      if (hot->maximized_vert)
        state_atoms[count++] = atoms._NET_WM_STATE_MAXIMIZED_VERT;
      if (wm_client_is_hidden(s, hot))
        state_atoms[count++] = atoms._NET_WM_STATE_HIDDEN;
      if (hot->flags & CLIENT_FLAG_FOCUSED)
        state_atoms[count++] = atoms._NET_WM_STATE_FOCUSED;

      xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, hot->xid,
                          atoms._NET_WM_STATE, XCB_ATOM_ATOM, 32, count,
                          state_atoms);
      ...
      bool fixed =
          (hot->hints.max_w > 0 && hot->hints.min_w == hot->hints.max_w &&
           hot->hints.max_h > 0 && hot->hints.min_h == hot->hints.max_h);
      ...
      xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, hot->xid,
                          atoms._NET_WM_ALLOWED_ACTIONS, XCB_ATOM_ATOM, 32,
                          num_actions, actions);
  ...
  if (desired_focus != s->committed_focus) {
    flushed = true;
    TRACE_LOG("flush_dirty commit focus %u -> %u", s->committed_focus,
              desired_focus);
    ...
    if (desired_focus == s->root) {
      ...
      xcb_set_input_focus(s->conn, XCB_INPUT_FOCUS_POINTER_ROOT, s->root,
                          XCB_CURRENT_TIME);
    } else if (focus_hot) {
      wm_install_client_colormap(s, focus_hot);

      if (focus_cold && focus_cold->can_focus) {
        xcb_set_input_focus(s->conn, XCB_INPUT_FOCUS_POINTER_ROOT,
                            focus_hot->xid, XCB_CURRENT_TIME);
      }
      ...
      if (focus_cold && (focus_cold->protocols & PROTOCOL_TAKE_FOCUS)) {
        ...
        ev.data.data32[1] =
            focus_hot->user_time ? focus_hot->user_time : XCB_CURRENT_TIME;
        xcb_send_event(s->conn, 0, focus_hot->xid, XCB_EVENT_MASK_NO_EVENT,
                       (const char *)&ev);
      }
    }
  ...
  if (s->root_dirty & ROOT_DIRTY_ACTIVE_WINDOW) {
    ...
    if (s->focused_client != HANDLE_INVALID) {
      ...
      if (c) {
        xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, s->root,
                            atoms._NET_ACTIVE_WINDOW, XCB_ATOM_WINDOW, 32, 1,
                            &c->xid);
      }
    } else {
  ...
  if (s->root_dirty &
      (ROOT_DIRTY_CLIENT_LIST | ROOT_DIRTY_CLIENT_LIST_STACKING)) {
    flushed = true;
    size_t cap = 0;
    for (int l = 0; l < LAYER_COUNT; l++)
      cap += s->layers[l].length;

    xcb_window_t *wins_stacking =
        cap ? (xcb_window_t *)arena_alloc(&s->tick_arena,
                                          cap * sizeof(xcb_window_t))
            : NULL;

    uint32_t idx_stacking = 0;
    if (wins_stacking) {
      idx_stacking =
          wm_build_client_list_stacking(s, wins_stacking, (uint32_t)cap);
    }
    ...
    xcb_window_t *wins_list =
        cap_list ? (xcb_window_t *)arena_alloc(&s->tick_arena,
                                               cap_list * sizeof(xcb_window_t))
                 : NULL;
    ...
    if (s->root_dirty & ROOT_DIRTY_CLIENT_LIST) {
      xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, s->root,
                          atoms._NET_CLIENT_LIST, XCB_ATOM_WINDOW, 32, idx_list,
                          wins_list);
    }

    if (s->root_dirty & ROOT_DIRTY_CLIENT_LIST_STACKING) {
      xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, s->root,
                          atoms._NET_CLIENT_LIST_STACKING, XCB_ATOM_WINDOW, 32,
                          idx_stacking, wins_stacking);
    }
  ...
  if (s->root_dirty & ROOT_DIRTY_CURRENT_DESKTOP) {
    flushed = true;
    xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, s->root,
                        atoms._NET_CURRENT_DESKTOP, XCB_ATOM_CARDINAL, 32, 1,
                        &s->current_desktop);
