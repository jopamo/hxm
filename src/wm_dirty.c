...
static void wm_send_sync_request(server_t *s, const client_hot_t *hot,
...
static void wm_sync_await(server_t *s, const client_hot_t *hot,
                          uint64_t value) {
...
void wm_flush_dirty(server_t *s) {
  ...
  for (uint32_t i = 1; i < slotmap_capacity(&s->clients); i++) {
    ...
    if (hot->dirty & DIRTY_GEOM) {
      ...
      bool do_sync = interactive_resize && hot->sync_enabled &&
      ...
      if (hot->desired.w < min_frame_w)
        hot->desired.w = (uint16_t)min_frame_w;
      if (hot->desired.h < min_frame_h)
        hot->desired.h = (uint16_t)min_frame_h;
      ...
      LOG_DEBUG("Flushed DIRTY_GEOM for %lx: %d,%d %dx%d", h, hot->pending.x,
                hot->pending.y, hot->pending.w, hot->pending.h);
    }
  ...
  if (sync_wait_client != HANDLE_INVALID) {
    ...
    if (wait_hot && wait_hot->sync_enabled &&
        wait_hot->sync_counter != XCB_NONE) {
