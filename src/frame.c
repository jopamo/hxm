...
void frame_init_resources(server_t *s) {
  ...
  for (size_t i = 0; i < sizeof(cursors) / sizeof(cursors[0]); i++) {
    *cursors[i].cursor = xcb_generate_id(s->conn);
    xcb_create_glyph_cursor(s->conn, *cursors[i].cursor, cursor_font,
                            cursor_font, cursors[i].id, cursors[i].id + 1, 0, 0,
...
static xcb_rectangle_t get_button_rect(server_t *s, client_hot_t *hot,
                                       frame_button_t btn) {
  uint16_t frame_w = hot->server.w + 2 * s->config.theme.border_width;
  // Buttons are laid out inside the right border
  int16_t x = (int16_t)frame_w - (int16_t)s->config.theme.border_width -
...
void frame_flush(server_t *s, handle_t h) {
  ...
  uint32_t f_dirty =
      hot->dirty & (DIRTY_FRAME_ALL | DIRTY_FRAME_TITLE | DIRTY_FRAME_BUTTONS |
                    DIRTY_FRAME_BORDER | DIRTY_TITLE | DIRTY_FRAME_STYLE);
  ...
  if (!(hot->dirty & (DIRTY_FRAME_ALL | DIRTY_FRAME_STYLE))) {
    if (hot->frame_damage.valid) {
      ...
      partial_clip = dirty_region_make(0, 0, frame_w,
      ...
      uint16_t btn_area_w =
          (uint16_t)(3 * (BUTTON_WIDTH + BUTTON_PADDING) + BUTTON_PADDING);
      ...
      int16_t clip_x =
          (int16_t)(frame_w - btn_area_w - s->config.theme.border_width);
      if (clip_x < 0)
        clip_x = 0;

      partial_clip = dirty_region_make(
          clip_x, 0, btn_area_w + s->config.theme.border_width,
          (uint16_t)s->config.theme.title_height);
      clip_ptr = &partial_clip;
    }
  }

  render_frame(s->conn, hot->frame, visual, &hot->render_ctx,
               (int)s->root_depth, s->is_test, cold ? cold->title : "", active,
               frame_w, frame_h, &s->config.theme,
               hot->icon_surface ? hot->icon_surface : s->default_icon,
               clip_ptr);
...
frame_button_t frame_get_button_at(server_t *s, handle_t h, int16_t x,
  ...
  frame_button_t btns[] = {FRAME_BUTTON_CLOSE, FRAME_BUTTON_MAXIMIZE,
