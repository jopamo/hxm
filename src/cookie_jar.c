...
bool cookie_jar_push(cookie_jar_t *cj, uint32_t sequence, cookie_type_t type,
                     handle_t client, uintptr_t data, uint64_t txn_id,
                     cookie_handler_fn handler) {
  // Grow before insertion to preserve probe behavior and keep load bounded
  if (cj->live_count * COOKIE_JAR_MAX_LOAD_DEN >=
      cj->cap * COOKIE_JAR_MAX_LOAD_NUM) {
 ...
 */
void cookie_jar_drain(cookie_jar_t *cj, xcb_connection_t *conn,
                      struct server *s, size_t max_replies) {
  ...
  while (scanned < cj->cap && processed < max_replies && cj->live_count > 0) {
    ...
    if (slot->live) {
      ...
      if (now > slot->timestamp_ns &&
          now - slot->timestamp_ns > COOKIE_JAR_TIMEOUT_NS) {
        cookie_slot_t local = *slot;
        cookie_jar_remove(cj, idx);
