...
static void client_queue_get_property(server_t *s, handle_t h, xcb_window_t win,
                                      xcb_atom_t prop, xcb_atom_t type,
                                      uint32_t long_len) {
  uint32_t seq =
      xcb_get_property(s->conn, 0, win, prop, type, 0, long_len).sequence;
  cookie_jar_push(&s->cookie_jar, seq, COOKIE_GET_PROPERTY, h,
                  ((uint64_t)win << 32) | prop, s->txn_id, wm_handle_reply);
}
...
bool should_focus_on_map(const client_hot_t *hot) {
  ...
  if (hot->type == WINDOW_TYPE_DOCK || hot->type == WINDOW_TYPE_NOTIFICATION ||
      hot->type == WINDOW_TYPE_DESKTOP || hot->type == WINDOW_TYPE_MENU ||
  ...
  if (hot->type == WINDOW_TYPE_DIALOG)
    return true;
...
static bool should_hide_for_show_desktop(const client_hot_t *hot) {
  return hot && hot->type != WINDOW_TYPE_DOCK &&
...
void client_manage_start(server_t *s, xcb_window_t win) {
  ...
  TRACE_LOG("manage_start init nodes h=%lx focus_node=%p", h,
            (void *)&hot->focus_node);
  ...
  cookie_jar_push(&s->cookie_jar, c2, COOKIE_GET_GEOMETRY, h, win, s->txn_id,
                  wm_handle_reply);

  for (size_t i = 0; i < ARRAY_LEN(props); i++) {
    client_queue_get_property(s, h, win, props[i].prop, props[i].type,
                              props[i].long_len);
  }
...
static void client_apply_rules(server_t *s, handle_t h) {
  ...
  for (size_t i = 0; i < s->config.rules.length; i++) {
    ...
    if (r->class_match &&
        (!cold->wm_class || strcmp(cold->wm_class, r->class_match) != 0))
      matched = false;
    if (matched && r->instance_match &&
        (!cold->wm_instance ||
         strcmp(cold->wm_instance, r->instance_match) != 0))
      matched = false;
    if (matched && r->title_match &&
        (!cold->title || !strstr(cold->title, r->title_match)))
      matched = false;
    if (matched && r->type_match != -1 && hot->type != (uint8_t)r->type_match)
      matched = false;
    if (matched && r->transient_match != -1) {
      bool is_transient = (hot->transient_for != HANDLE_INVALID);
      if (is_transient != (bool)r->transient_match)
        matched = false;
    }
    ...
    if (matched) {
      ...
      if (r->focus != -1)
        hot->focus_override = r->focus;
      if (r->placement != PLACEMENT_DEFAULT)
        hot->placement = (uint8_t)r->placement;
...
void client_finish_manage(server_t *s, handle_t h) {
  ...
  TRACE_LOG("finish_manage h=%lx xid=%u desktop=%d sticky=%d initial_state=%u",
            h, hot->xid, hot->desktop, hot->sticky, hot->initial_state);
  ...
  xcb_change_window_attributes(s->conn, hot->xid, XCB_CW_EVENT_MASK,
                               &client_events);
  ...
  values[1] = XCB_EVENT_MASK_SUBSTRUCTURE_NOTIFY | XCB_EVENT_MASK_EXPOSURE |
  ...
  if (hot->original_border_width != 0) {
    ...
    xcb_configure_window(s->conn, hot->xid, XCB_CONFIG_WINDOW_BORDER_WIDTH,
                         bw_values);
  }
  ...
  if (hot->window_opacity_valid) {
    xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, hot->frame,
                        atoms._NET_WM_WINDOW_OPACITY, XCB_ATOM_CARDINAL, 32, 1,
                        &hot->window_opacity);
  }

  if (hot->bypass_compositor_valid) {
    xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, hot->frame,
                        atoms._NET_WM_BYPASS_COMPOSITOR, XCB_ATOM_CARDINAL, 32,
                        1, &hot->bypass_compositor);
  }
  ...
  bool fixed = (hot->hints.max_w > 0 && hot->hints.min_w == hot->hints.max_w &&
  ...
  bool visible =
      (hot->sticky || (hot->desktop == (int32_t)s->current_desktop)) &&
      (hot->initial_state != XCB_ICCCM_WM_STATE_ICONIC);

  TRACE_LOG("finish_manage visibility h=%lx visible=%d current_desktop=%u", h,
            visible, s->current_desktop);
  ...
  if (s->showing_desktop && hot->state == STATE_MAPPED &&
      should_hide_for_show_desktop(hot)) {
  ...
  if (s->damage_supported) {
    ...
    xcb_damage_create(s->conn, hot->damage, hot->xid,
  ...
  if (!hidden_by_show_desktop) {
    ...
    if (visible) {
      if (s->initial_focus != XCB_NONE && hot->xid == s->initial_focus) {
        ...
        focus_it = true;
      } else if (s->focused_client == HANDLE_INVALID &&
    ...
    if (hot->transient_for != HANDLE_INVALID) {
      ...
      if (!focus_it || !s->config.focus_raise) {
        TRACE_LOG("finish_manage stack above parent h=%lx parent=%lx", h,
                  hot->transient_for);
  ...
  small_vec_t *stashed =
      (small_vec_t *)hash_map_get(&s->pending_unmanaged_states, hot->xid);
  if (stashed) {
    LOG_INFO("Replaying %zu stashed _NET_WM_STATE messages for client %lx",
             stashed->length, h);
    for (size_t i = 0; i < stashed->length; i++) {
      pending_state_msg_t *msg = (pending_state_msg_t *)stashed->items[i];
      if (msg) {
        TRACE_LOG("Replaying stashed _NET_WM_STATE action=%u p1=%u p2=%u",
                  msg->action, msg->p1, msg->p2);
  ...
  if (hot->pending_state_count > 0) {
    LOG_INFO("Replaying %u queued _NET_WM_STATE messages for client %lx",
             hot->pending_state_count, h);
    for (int i = 0; i < hot->pending_state_count; ++i) {
      pending_state_msg_t *msg = &hot->pending_state_msgs[i];
      TRACE_LOG("Replaying _NET_WM_STATE action=%u p1=%u p2=%u", msg->action,
                msg->p1, msg->p2);
      if (msg->p1 != XCB_ATOM_NONE)
        wm_client_update_state(s, h, msg->action, msg->p1);
      if (msg->p2 != XCB_ATOM_NONE)
...
void client_unmanage(server_t *s, handle_t h) {
  ...
  LOG_INFO("Unmanaging client %lx (window %u, destroyed=%d)", h, hot->xid,
           destroyed);
  TRACE_LOG("unmanage h=%lx frame=%u state=%d ignore_unmap=%u", h, hot->frame,
            hot->state, hot->ignore_unmap);
  ...
  if (s->interaction_mode != INTERACTION_NONE && s->interaction_handle == h) {
    LOG_INFO("Interaction client %lx destroyed/unmanaged during interaction, "
             "canceling",
             h);
  ...
  // Unlink from parent
  if (hot->transient_sibling.next &&
      hot->transient_sibling.next != &hot->transient_sibling) {
  ...
  while (!list_empty(&hot->transients_head)) {
    ...
    client_hot_t *child =
        (client_hot_t *)((char *)node -
                         offsetof(client_hot_t, transient_sibling));
  ...
  if (hot->focus_node.next && hot->focus_node.next != &hot->focus_node) {
    TRACE_LOG("unmanage focus_history remove h=%lx node=%p prev=%p next=%p", h,
              (void *)&hot->focus_node, (void *)hot->focus_node.prev,
              (void *)hot->focus_node.next);
    list_remove(&hot->focus_node);
    list_init(&hot->focus_node);
  }
...
#ifdef HXM_DEBUG_TRACE
        if (guard < 64) {
          TRACE_LOG("unmanage focus scan[%d] node=%p h=%lx xid=%u state=%d",
                    guard, (void *)node, cand->self, cand->xid, cand->state);
        } else if (guard == 64) {
  ...
  if (!destroyed) {
    ...
    TRACE_LOG("unmanage reparent xid=%u -> root (%d,%d)", hot->xid, root_x,
              root_y);
...
void client_close(server_t *s, handle_t h) {
  client_hot_t *hot = server_chot(s, h);
  client_cold_t *cold = server_ccold(s, h);
  if (!hot || !cold || hot->state == STATE_DESTROYED ||
      hot->state == STATE_UNMANAGED)
    return;
...
void client_constrain_size(const size_hints_t *s, uint32_t flags, uint16_t *w,
                           uint16_t *h) {
  // Min/Max size
  ...
  if (flags & XCB_ICCCM_SIZE_HINT_P_ASPECT) {
    if (s->min_aspect_den > 0 && s->min_aspect_num > 0) {
      if ((int32_t)(*w) * s->min_aspect_den <
          (int32_t)(*h) * s->min_aspect_num) {
        *w = (uint16_t)((int32_t)(*h) * s->min_aspect_num / s->min_aspect_den);
      }
    }
    if (s->max_aspect_den > 0 && s->max_aspect_num > 0) {
      if ((int32_t)(*w) * s->max_aspect_den >
          (int32_t)(*h) * s->max_aspect_num) {
  ...
  if (flags & XCB_ICCCM_SIZE_HINT_P_RESIZE_INC) {
    if (s->inc_w > 1) {
      int32_t base_w = (flags & XCB_ICCCM_SIZE_HINT_BASE_SIZE)
                           ? s->base_w
                           : (s->min_w > 0 ? s->min_w : 0);
      if (*w > base_w) {
    ...
    if (s->inc_h > 1) {
      int32_t base_h = (flags & XCB_ICCCM_SIZE_HINT_BASE_SIZE)
                           ? s->base_h
                           : (s->min_h > 0 ? s->min_h : 0);
      if (*h > base_h) {
...
void client_setup_grabs(server_t *s, handle_t h) {
  ...
  for (int i = 0; i < 3; i++) {
    xcb_grab_button(s->conn, 0, hot->xid, XCB_EVENT_MASK_BUTTON_PRESS,
                    XCB_GRAB_MODE_SYNC, XCB_GRAB_MODE_ASYNC, XCB_NONE, XCB_NONE,
                    buttons[i], XCB_MOD_MASK_ANY);
  }
}
...
bool client_can_resize(const client_hot_t *hot) {
  ...
  if ((hot->hints_flags & XCB_ICCCM_SIZE_HINT_P_MIN_SIZE) &&
      (hot->hints_flags & XCB_ICCCM_SIZE_HINT_P_MAX_SIZE)) {
    if (hot->hints.min_w > 0 && hot->hints.min_w == hot->hints.max_w &&
        hot->hints.min_h > 0 && hot->hints.min_h == hot->hints.max_h) {
      return false;
    }
  }
  return true;
