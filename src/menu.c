...
static void menu_format_client_label(const server_t *s, const client_hot_t *hot,
                                     const client_cold_t *cold, char *out,
                                     size_t out_len) {
  ...
  if (hot && !hot->sticky && hot->desktop >= 0 &&
    ...
    pos +=
        (size_t)snprintf(tag + pos, sizeof(tag) - pos, "ws %d", hot->desktop);
  }
...
static cairo_surface_t *menu_load_icon(const char *path) {
  ...
  if (is_svg) {
    ...
    if (!ok || error) {
      LOG_WARN("Failed to render SVG icon %s: %s", path,
...
static void menu_add_item(server_t *s, const char *label, menu_action_t action,
                          const char *cmd, handle_t client,
...
void menu_init(server_t *s) {
  ...
  uint32_t values[] = {
      s->config.theme.menu_items.color, 1,
      XCB_EVENT_MASK_EXPOSURE | XCB_EVENT_MASK_BUTTON_PRESS |
          XCB_EVENT_MASK_BUTTON_RELEASE | XCB_EVENT_MASK_POINTER_MOTION |
          XCB_EVENT_MASK_KEY_PRESS | XCB_EVENT_MASK_KEY_RELEASE};

  xcb_create_window(s->conn, XCB_COPY_FROM_PARENT, s->menu.window, s->root, 0,
                    0, s->menu.w, s->menu.h, 1, XCB_WINDOW_CLASS_INPUT_OUTPUT,
...
static void menu_populate_root(server_t *s) {
  ...
  if (s->menu.config_items.length == 0) {
    menu_add_item(s, "(Menu not configured)", MENU_ACTION_NONE, NULL,
                  HANDLE_INVALID, NULL);
    return;
  }
  ...
  for (size_t i = 0; i < s->menu.config_items.length; i++) {
    ...
    if (!spec)
      continue;
    const char *label =
        (spec->action == MENU_ACTION_SEPARATOR) ? NULL : spec->label;
    menu_add_item(s, label, spec->action, spec->cmd, HANDLE_INVALID,
                  spec->icon_path);
  }
}
...
void menu_show(server_t *s, int16_t x, int16_t y) {
  ...
  xcb_configure_window(s->conn, s->menu.window, XCB_CONFIG_WINDOW_HEIGHT,
                       values_h);
  ...
  xcb_configure_window(s->conn, s->menu.window,
                       XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y, values);

  xcb_map_window(s->conn, s->menu.window);

  const uint32_t stack_values[] = {XCB_STACK_MODE_ABOVE};
  xcb_configure_window(s->conn, s->menu.window, XCB_CONFIG_WINDOW_STACK_MODE,
                       stack_values);
  ...
  xcb_grab_keyboard_cookie_t kc =
      xcb_grab_keyboard(s->conn, 0, s->root, XCB_CURRENT_TIME,
...
void menu_show_client_list(server_t *s, int16_t x, int16_t y) {
  ...
  if (s->menu.items.length == 0) {
    menu_add_item(s, "(No windows)", MENU_ACTION_NONE, NULL, HANDLE_INVALID,
                  NULL);
  }
  ...
  if (pr) {
    if (pr->status != XCB_GRAB_STATUS_SUCCESS) {
      LOG_WARN("menu_show_client_list: pointer grab failed status=%d",
               pr->status);
    }
    free(pr);
  }
  ...
  if (kr) {
    if (kr->status != XCB_GRAB_STATUS_SUCCESS) {
      LOG_WARN("menu_show_client_list: keyboard grab failed status=%d",
               kr->status);
    }
    free(kr);
  }
...
static rgba_t u32_to_rgba(uint32_t c) {
  return (rgba_t){((c >> 16) & 0xFF) / 255.0, ((c >> 8) & 0xFF) / 255.0,
                  (c & 0xFF) / 255.0, 1.0};
}
...
void menu_handle_expose_region(server_t *s, const dirty_region_t *dirty) {
  ...
  if (s->is_test) {
    target_surface = cairo_image_surface_create(
        (s->root_depth == 32) ? CAIRO_FORMAT_ARGB32 : CAIRO_FORMAT_RGB24,
        s->menu.w, s->menu.h);
  } else {
    pixmap = xcb_generate_id(s->conn);
    xcb_create_pixmap(s->conn, s->root_depth, pixmap, s->menu.window, s->menu.w,
                      s->menu.h);
    target_surface = cairo_xcb_surface_create(
        s->conn, pixmap, s->root_visual_type, s->menu.w, s->menu.h);
  }
  ...
  for (size_t i = 0; i < s->menu.items.length; i++) {
    ...
    if (item->action == MENU_ACTION_SEPARATOR) {
      ...
      cairo_line_to(cr, s->menu.w - MENU_PADDING,
    ...
    cairo_set_source_rgba(cr, text_color.r, text_color.g, text_color.b,
                          text_color.a);
    pango_layout_set_text(s->menu.render_ctx.layout,
                          item->label ? item->label : "", -1);
    pango_layout_set_width(s->menu.render_ctx.layout,
                           (s->menu.w - text_x_offset - MENU_PADDING) *
                               PANGO_SCALE);
  ...
  if (s->is_test) {
    ...
    xcb_put_image(
        s->conn, XCB_IMAGE_FORMAT_Z_PIXMAP, s->menu.window, gc,
        (uint16_t)s->menu.w, (uint16_t)s->menu.h, 0, 0, 0,
        (uint8_t)s->root_depth,
    ...
    uint32_t values[] = {0};
    xcb_create_gc(s->conn, gc, s->menu.window, mask, values);
    xcb_copy_area(s->conn, pixmap, s->menu.window, gc, 0, 0, 0, 0, s->menu.w,
                  s->menu.h);
    xcb_free_gc(s->conn, gc);
    xcb_free_pixmap(s->conn, pixmap);
  }
...
void menu_show_switcher(server_t *s, handle_t origin) {
  ...
  rect_t geom;
  client_hot_t *origin_hot =
      (origin != HANDLE_INVALID) ? server_chot(s, origin) : NULL;
  if (origin_hot) {
  ...
  if (kr) {
    if (kr->status != XCB_GRAB_STATUS_SUCCESS) {
      LOG_WARN("menu_show_switcher: keyboard grab failed status=%d",
               kr->status);
    }
    free(kr);
  }
...
void menu_handle_pointer_motion(server_t *s, int16_t x, int16_t y) {
  ...
  if (local_x < 0 || local_x >= (int32_t)s->menu.w || local_y < 0 ||
      local_y >= (int32_t)s->menu.h) {
...
static void menu_activate_selected(server_t *s) {
  ...
  if (s->menu.selected_index < 0 ||
      s->menu.selected_index >= (int)s->menu.items.length)
    return;
