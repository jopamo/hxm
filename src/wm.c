...
static bool check_wm_s0_available(server_t *s) {
  xcb_get_selection_owner_cookie_t cookie =
      xcb_get_selection_owner(s->conn, atoms.WM_S0);
  xcb_get_selection_owner_reply_t *reply =
      xcb_get_selection_owner_reply(s->conn, cookie, NULL);
  if (!reply)
    return false;
...
static void set_root_cursor(server_t *s, uint16_t cursor_font_id) {
  ...
  xcb_create_glyph_cursor(s->conn, cursor, font, font, cursor_font_id,
                          cursor_font_id + 1, 0, 0, 0, 0xffff, 0xffff, 0xffff);
...
static bool wm_is_above_in_layer(const server_t *s, const client_hot_t *a,
                                 const client_hot_t *b) {
  if (!a || !b)
    return false;
  if (a->layer != b->layer)
    return false;
...
void wm_set_frame_extents_for_window(server_t *s, xcb_window_t win,
                                     bool undecorated) {
...
void wm_update_monitors(server_t *s) {
  ...
  if (!s->randr_supported) {
    ...
    xcb_randr_get_screen_resources_current_cookie_t r_c =
        xcb_randr_get_screen_resources_current(s->conn, s->root);
    ...
    if (res) {
      ...
      for (int i = 0; i < num_crtcs; i++) {
        ...
        xcb_randr_get_crtc_info_reply_t *crtc =
            xcb_randr_get_crtc_info_reply(s->conn, c_c, NULL);
        ...
        if (crtc) {
          if (crtc->mode != XCB_NONE) {
            ...
            next_monitors[active_count].workarea =
                next_monitors[active_count].geom;
            active_count++;
          }
          free(crtc);
        }
...
void wm_get_monitor_geometry(server_t *s, client_hot_t *hot, rect_t *out_geom) {
  ...
  for (uint32_t i = 0; i < s->monitor_count; i++) {
    ...
    if (center_x >= m->geom.x && center_x < m->geom.x + (int)m->geom.w &&
        center_y >= m->geom.y && center_y < m->geom.y + (int)m->geom.h) {
      *out_geom = m->geom;
      break;
    }
  }
...
void wm_client_set_maximize(server_t *s, client_hot_t *hot, bool max_horz,
                            bool max_vert) {
  if (!hot)
    return;
...
void wm_become(server_t *s) {
  ...
  xcb_void_cookie_t cwa = xcb_change_window_attributes_checked(
      conn, root, XCB_CW_EVENT_MASK, &root_events);
  xcb_generic_error_t *err = xcb_request_check(conn, cwa);
  if (err) {
    // If this hits, it's usually BadAccess (another WM)
    LOG_ERROR("Failed to select root events (likely another WM). error_code=%u",
              err->error_code);
    free(err);
    return;
  }
  ...
  xcb_change_property(
      conn, XCB_PROP_MODE_REPLACE, root, atoms._NET_SUPPORTED, XCB_ATOM_ATOM,
      32, (uint32_t)(sizeof(supported_atoms) / sizeof(supported_atoms[0])),
      supported_atoms);
  ...
  xcb_create_window(conn, XCB_COPY_FROM_PARENT, s->supporting_wm_check, root, 0,
                    0, 1, 1, 0, XCB_WINDOW_CLASS_INPUT_OUTPUT,
                    XCB_COPY_FROM_PARENT, mask, values);

  // Acquire WM_S0 selection (EWMH)
  xcb_set_selection_owner(conn, s->supporting_wm_check, atoms.WM_S0,
                          XCB_CURRENT_TIME);

  // Verify ownership (defensive)
  {
    xcb_get_selection_owner_cookie_t ck =
        xcb_get_selection_owner(conn, atoms.WM_S0);
  ...
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root,
                      atoms._NET_SUPPORTING_WM_CHECK, XCB_ATOM_WINDOW, 32, 1,
                      &s->supporting_wm_check);
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, s->supporting_wm_check,
                      atoms._NET_SUPPORTING_WM_CHECK, XCB_ATOM_WINDOW, 32, 1,
                      &s->supporting_wm_check);
  ...
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root, atoms._NET_WM_NAME,
                      atoms.UTF8_STRING, 8, (uint32_t)strlen(wm_name), wm_name);
  ...
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root, atoms._NET_WM_PID,
  ...
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root,
                      atoms._NET_DESKTOP_GEOMETRY, XCB_ATOM_CARDINAL, 32, 2,
                      geometry);
  ...
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root, atoms._NET_CLIENT_LIST,
                      XCB_ATOM_WINDOW, 32, 0, NULL);
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root,
                      atoms._NET_CLIENT_LIST_STACKING, XCB_ATOM_WINDOW, 32, 0,
                      NULL);
  xcb_delete_property(conn, root, atoms._NET_ACTIVE_WINDOW);
  {
    uint32_t val = 0;
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root,
                        atoms._NET_SHOWING_DESKTOP, XCB_ATOM_CARDINAL, 32, 1,
                        &val);
  }
...
void wm_adopt_children(server_t *s) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    xcb_get_window_attributes_cookie_t ck =
        xcb_get_window_attributes(s->conn, win);
    cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_GET_WINDOW_ATTRIBUTES,
                    HANDLE_INVALID, (uint64_t)win, s->txn_id, wm_handle_reply);
  }
...
void wm_handle_map_request(server_t *s, xcb_map_request_event_t *ev) {
  ...
  if (h != HANDLE_INVALID) {
    ...
    if (hot && hot->state == STATE_UNMAPPED) {
      ...
      LOG_DEBUG("Ignoring MapRequest for already managed client %u (state %d)",
                ev->window, hot ? hot->state : -1);
    }
    return;
  }
  ...
  xcb_get_window_attributes_cookie_t ck =
      xcb_get_window_attributes(s->conn, ev->window);
  cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_CHECK_MANAGE_MAP_REQUEST,
                  HANDLE_INVALID, (uint64_t)ev->window, s->txn_id,
                  wm_handle_reply);
}

void wm_handle_unmap_notify(server_t *s, xcb_unmap_notify_event_t *ev) {
  TRACE_LOG("unmap_notify win=%u event=%u from_configure=%u", ev->window,
            ev->event, ev->from_configure);
  handle_t h = server_get_client_by_window(s, ev->window);
  if (h == HANDLE_INVALID) {
  ...
  if (hot->ignore_unmap > 0 &&
      (ev->window == hot->xid || ev->window == hot->frame)) {
    TRACE_LOG("unmap_notify ignore h=%lx win=%u count=%u", h, ev->window,
              hot->ignore_unmap);
  ...
  TRACE_LOG("unmap_notify unmanage h=%lx xid=%u frame=%u", h, hot->xid,
            hot->frame);
  client_unmanage(s, h);
}
...
void wm_handle_destroy_notify(server_t *s, xcb_destroy_notify_event_t *ev) {
  ...
  small_vec_t *v =
      (small_vec_t *)hash_map_get(&s->pending_unmanaged_states, ev->window);
  if (v) {
    for (size_t i = 0; i < v->length; i++)
      free(v->items[i]);
    small_vec_destroy(v);
  ...
  if (hot) {
    ...
    if (ev->window == hot->frame) {
      if (hot->state == STATE_MAPPED) {
        LOG_WARN(
            "Frame %u destroyed for managed client %lx. Marking frame dead.",
            ev->window, h);
 ...
 */
void wm_handle_configure_request(server_t *s, handle_t h,
                                 pending_config_t *ev) {
  client_hot_t *hot = server_chot(s, h);
  if (!hot)
    return;
  ...
  TRACE_LOG(
      "configure_request h=%lx xid=%u mask=0x%x x=%d y=%d w=%u h=%u bw=%u", h,
      hot->xid, ev->mask, ev->x, ev->y, ev->width, ev->height,
      ev->border_width);
  ...
  bool is_panel =
      (hot->type == WINDOW_TYPE_DOCK || hot->type == WINDOW_TYPE_DESKTOP);
  if (!is_panel) {
  ...
  LOG_DEBUG("Client %lx desired geom updated: %d,%d %dx%d (mask %x)", h,
            hot->desired.x, hot->desired.y, hot->desired.w, hot->desired.h,
            ev->mask);
}

void wm_handle_configure_notify(server_t *s, handle_t h,
  ...
  TRACE_LOG("configure_notify h=%lx win=%u x=%d y=%d w=%u h=%u bw=%u above=%u",
            h, ev->window, ev->x, ev->y, ev->width, ev->height,
            ev->border_width, ev->above_sibling);
  ...
  if (ev->window == hot->frame) {
    ...
    LOG_DEBUG("Client %lx window size updated: %dx%d", h, ev->width,
              ev->height);
  }
}

void wm_handle_property_notify(server_t *s, handle_t h,
                               xcb_property_notify_event_t *ev) {
  if (ev->atom == atoms._NET_WM_ALLOWED_ACTIONS ||
      ev->atom == atoms._NET_FRAME_EXTENTS || ev->atom == atoms.WM_STATE ||
      ev->atom == atoms._NET_WM_VISIBLE_NAME ||
  ...
  TRACE_LOG("property_notify h=%lx xid=%u atom=%u (%s) state=%u", h, hot->xid,
            ev->atom, atom_name(ev->atom), ev->state);
  if (ev->atom == atoms.WM_NAME || ev->atom == atoms._NET_WM_NAME) {
    ...
    xcb_get_property_cookie_t ck = xcb_get_property(
        s->conn, 0, hot->xid, atoms.WM_PROTOCOLS, XCB_ATOM_ATOM, 0, 32);
    cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_GET_PROPERTY, h,
                    ((uint64_t)hot->xid << 32) | atoms.WM_PROTOCOLS, s->txn_id,
                    wm_handle_reply);
  } else if (ev->atom == atoms._NET_WM_ICON) {
    xcb_get_property_cookie_t ck =
        xcb_get_property(s->conn, 0, hot->xid, atoms._NET_WM_ICON,
                         XCB_ATOM_CARDINAL, 0, 1048576);
    cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_GET_PROPERTY, h,
                    ((uint64_t)hot->xid << 32) | atoms._NET_WM_ICON, s->txn_id,
                    wm_handle_reply);
  } else if (ev->atom == atoms._NET_WM_STATE) {
    xcb_get_property_cookie_t ck = xcb_get_property(
        s->conn, 0, hot->xid, atoms._NET_WM_STATE, XCB_ATOM_ATOM, 0, 32);
    cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_GET_PROPERTY, h,
                    ((uint64_t)hot->xid << 32) | atoms._NET_WM_STATE, s->txn_id,
                    wm_handle_reply);
  } else if (ev->atom == atoms._NET_WM_WINDOW_TYPE) {
    xcb_get_property_cookie_t ck = xcb_get_property(
        s->conn, 0, hot->xid, atoms._NET_WM_WINDOW_TYPE, XCB_ATOM_ATOM, 0, 32);
    cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_GET_PROPERTY, h,
                    ((uint64_t)hot->xid << 32) | atoms._NET_WM_WINDOW_TYPE,
                    s->txn_id, wm_handle_reply);
  } else if (ev->atom == atoms._NET_WM_DESKTOP) {
    xcb_get_property_cookie_t ck = xcb_get_property(
        s->conn, 0, hot->xid, atoms._NET_WM_DESKTOP, XCB_ATOM_CARDINAL, 0, 1);
    cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_GET_PROPERTY, h,
                    ((uint64_t)hot->xid << 32) | atoms._NET_WM_DESKTOP,
                    s->txn_id, wm_handle_reply);
  } else if (ev->atom == atoms._NET_WM_SYNC_REQUEST_COUNTER) {
    xcb_get_property_cookie_t ck = xcb_get_property(
        s->conn, 0, hot->xid, atoms._NET_WM_SYNC_REQUEST_COUNTER,
        XCB_ATOM_CARDINAL, 0, 1);
    cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_GET_PROPERTY, h,
                    ((uint64_t)hot->xid << 32) |
                        atoms._NET_WM_SYNC_REQUEST_COUNTER,
                    s->txn_id, wm_handle_reply);
  } else if (ev->atom == atoms._NET_WM_STRUT ||
             ev->atom == atoms._NET_WM_STRUT_PARTIAL) {
    hot->dirty |= DIRTY_STRUT;
    // Waterfall: Always request PARTIAL first. If it fails/empty, we fallback
    // to STRUT in reply handler
    xcb_get_property_cookie_t ck =
        xcb_get_property(s->conn, 0, hot->xid, atoms._NET_WM_STRUT_PARTIAL,
                         XCB_ATOM_CARDINAL, 0, 12);
    cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_GET_PROPERTY, h,
                    ((uint64_t)hot->xid << 32) |
                        (uint32_t)atoms._NET_WM_STRUT_PARTIAL,
                    s->txn_id, wm_handle_reply);
...
static bool colormap_list_contains(const client_cold_t *cold,
                                   xcb_window_t win) {
...
void wm_client_apply_state_set(server_t *s, handle_t h,
                               const client_state_set_t *set) {
  client_hot_t *hot = server_chot(s, h);
  if (!hot || !set)
    return;

  if (hot->maximized_horz != set->max_horz ||
      hot->maximized_vert != set->max_vert) {
...
void wm_start_interaction(server_t *s, handle_t h, client_hot_t *hot,
  ...
  xcb_grab_pointer_cookie_t cookie = xcb_grab_pointer(
      ...
      XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC, XCB_NONE, cursor,
      time ? time : XCB_CURRENT_TIME);

  xcb_grab_pointer_reply_t *reply =
      xcb_grab_pointer_reply(s->conn, cookie, NULL);
  bool success = false;
  if (reply) {
    if (reply->status == XCB_GRAB_STATUS_SUCCESS) {
      success = true;
      LOG_INFO("grab_pointer success status=%u on root=%u", reply->status,
               s->root);
    } else {
      LOG_ERROR("grab_pointer failed status=%u on root=%u", reply->status,
                s->root);
    }
    free(reply);
  } else {
  ...
  LOG_INFO("Started interactive %s for client %lx (dir=%d)",
           start_move ? "MOVE" : "RESIZE", h, resize_dir);
}
...
void wm_handle_button_press(server_t *s, xcb_button_press_event_t *ev) {
  ...
  if (s->focused_client != h && hot->type != WINDOW_TYPE_DOCK &&
      ...
      hot->type != WINDOW_TYPE_COMBO && hot->type != WINDOW_TYPE_DND) {
    wm_set_focus(s, h);
  ...
  wm_start_interaction(s, h, hot, start_move, resize_dir, ev->root_x,
                       ev->root_y, ev->time, false);
...
void wm_handle_button_release(server_t *s, xcb_button_release_event_t *ev) {
  ...
  if (s->interaction_mode == INTERACTION_MOVE) {
    ...
    if (hot) {
      bool can_snap =
          s->snap_enabled && client_can_move(hot) && !hot->override_redirect &&
...
void wm_handle_motion_notify(server_t *s, xcb_motion_notify_event_t *ev) {
  ...
  LOG_DEBUG("MotionNotify interactive: root=%d,%d event=%u child=%u",
            ev->root_x, ev->root_y, ev->event, ev->child);
  ...
  if (s->interaction_requires_buttons &&
      !(ev->state & (XCB_KEY_BUT_MASK_BUTTON_1 | XCB_KEY_BUT_MASK_BUTTON_2 |
                     XCB_KEY_BUT_MASK_BUTTON_3 | XCB_KEY_BUT_MASK_BUTTON_4 |
                     XCB_KEY_BUT_MASK_BUTTON_5))) {
    LOG_INFO("MotionNotify with no buttons down (state=0x%x): forcing "
             "interaction end",
             ev->state);
    wm_cancel_interaction(s);
    return;
  ...
  client_hot_t *hot = server_chot(s, h);
  if (!hot) {
    LOG_WARN("Interaction client not found h=%lx window=%u", h,
             s->interaction_window);
    s->interaction_mode = INTERACTION_NONE;
    xcb_ungrab_pointer(s->conn, XCB_CURRENT_TIME);
    return;
  }
  ...
  if (s->interaction_mode == INTERACTION_MOVE) {
    ...
    if (s->snap_enabled && client_can_move(hot) && !hot->override_redirect &&
        hot->layer != LAYER_FULLSCREEN && hot->type != WINDOW_TYPE_DOCK) {
      ...
      snap_candidate_t cand = snap_compute_candidate(ev->root_x, ev->root_y, wa,
                                                     (int)s->snap_threshold_px);
      if (cand.active) {
...
void wm_client_update_state(server_t *s, handle_t h, uint32_t action,
                            xcb_atom_t prop) {
  client_hot_t *hot = server_chot(s, h);
  if (!hot)
    return;
  ...
  if (prop == atoms._NET_WM_STATE_HIDDEN) {
    ...
    if (hot->state == STATE_NEW) {
      LOG_DEBUG(
          "Ignoring _NET_WM_STATE_HIDDEN change for client %lx in STATE_NEW",
          h);
      return;
    }
    ...
    if (action == 2) {
      if (hot->state == STATE_MAPPED) {
        ...
        LOG_DEBUG(
            "wm_client_update_state: Restoring client %lx (action=2, hidden)",
            h);
        wm_client_restore(s, h);
      }
    } else if (action == 1) {
      ...
      if (hot->state == STATE_UNMAPPED) {
        LOG_DEBUG(
            "wm_client_update_state: Restoring client %lx (action=0, hidden)",
            h);
        wm_client_restore(s, h);
      }
    }
  ...
  if (prop == atoms._NET_WM_STATE_FULLSCREEN) {
    if (add && hot->layer != LAYER_FULLSCREEN) {
      ...
      if (hot->fullscreen_monitors_valid) {
        xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, hot->xid,
                            atoms._NET_WM_FULLSCREEN_MONITORS,
                            XCB_ATOM_CARDINAL, 32, 4, hot->fullscreen_monitors);
      } else {
        xcb_delete_property(s->conn, hot->xid,
                            atoms._NET_WM_FULLSCREEN_MONITORS);
      }
      ...
      hot->flags = (hot->flags & ~CLIENT_FLAG_UNDECORATED) |
                   (hot->saved_state_mask & CLIENT_FLAG_UNDECORATED);
  ...
  if (prop == atoms._NET_WM_STATE_MAXIMIZED_HORZ ||
      prop == atoms._NET_WM_STATE_MAXIMIZED_VERT) {
...
void wm_handle_client_message(server_t *s, xcb_client_message_event_t *ev) {
  TRACE_ONLY({
    TRACE_LOG(
        "client_message type=%u window=%u format=%u data=[%u,%u,%u,%u,%u]",
        ev->type, ev->window, ev->format, ev->data.data32[0],
        ev->data.data32[1], ev->data.data32[2], ev->data.data32[3],
        ev->data.data32[4]);
  });
  if (ev->type == atoms.WM_PROTOCOLS && ev->window == s->root &&
      ev->format == 32 && ev->data.data32[0] == atoms._NET_WM_PING) {
  ...
  if (ev->type == atoms.WM_CHANGE_STATE && ev->format == 32) {
    ...
    if (h != HANDLE_INVALID) {
      ...
      if (state == XCB_ICCCM_WM_STATE_ICONIC) {
        ...
        LOG_DEBUG(
            "wm_handle_client_message: Restoring client %lx (WM_CHANGE_STATE)",
            h);
        wm_client_restore(s, h);
      }
    }
  ...
  if (ev->type == atoms._NET_DESKTOP_GEOMETRY) {
    if (ev->window != s->root || ev->format != 32)
      return;
    TRACE_LOG("_NET_DESKTOP_GEOMETRY request=%u,%u", ev->data.data32[0],
              ev->data.data32[1]);
    xcb_screen_t *screen =
        xcb_setup_roots_iterator(xcb_get_setup(s->conn)).data;
    uint32_t geometry[] = {screen->width_in_pixels, screen->height_in_pixels};
    xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, s->root,
                        atoms._NET_DESKTOP_GEOMETRY, XCB_ATOM_CARDINAL, 32, 2,
                        geometry);
    return;
  }
  ...
  if (ev->type == atoms._NET_DESKTOP_VIEWPORT) {
    if (ev->window != s->root || ev->format != 32)
      return;
    TRACE_LOG("_NET_DESKTOP_VIEWPORT request=%u,%u", ev->data.data32[0],
              ev->data.data32[1]);
    uint32_t *viewport = calloc(s->desktop_count * 2, sizeof(uint32_t));
    if (viewport) {
      xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, s->root,
                          atoms._NET_DESKTOP_VIEWPORT, XCB_ATOM_CARDINAL, 32,
  ...
  if (ev->type == atoms._NET_ACTIVE_WINDOW) {
    ...
    if (h != HANDLE_INVALID) {
      ...
      if (!hot)
        return;
      TRACE_LOG("_NET_ACTIVE_WINDOW h=%lx xid=%u state=%d desktop=%d", h,
                hot->xid, hot->state, hot->desktop);

      if (hot->manage_phase != MANAGE_DONE || hot->frame == XCB_WINDOW_NONE) {
        LOG_DEBUG(
            "Ignoring _NET_ACTIVE_WINDOW for incomplete client %lx phase=%d", h,
            hot->manage_phase);
        return;
      }

      if (!hot->sticky && hot->desktop >= 0 &&
          (uint32_t)hot->desktop != s->current_desktop) {
        wm_switch_workspace(s, (uint32_t)hot->desktop);
      }
      bool was_unmapped = (hot->state == STATE_UNMAPPED);
      if (was_unmapped) {
        LOG_DEBUG("wm_handle_client_message: Restoring client %lx "
                  "(NET_ACTIVE_WINDOW)",
                  h);
        wm_client_restore(s, h);
      }
  ...
  if (ev->type == atoms._NET_WM_STATE) {
    ...
    TRACE_LOG("_NET_WM_STATE h=%lx win=%u action=%u p1=%s(%u) p2=%s(%u)", h,
              ev->window, action, atom_name(p1), p1, atom_name(p2), p2);
    ...
    if (h != HANDLE_INVALID) {
      ...
      if (p1 != XCB_ATOM_NONE) {
        if (p1 == atoms._NET_WM_STATE_HIDDEN) {
          LOG_DEBUG(
              "Ignoring client request for _NET_WM_STATE_HIDDEN (WM owned)");
        } else {
  ...
  if (ev->type == atoms._NET_WM_FULLSCREEN_MONITORS) {
    ...
    if (!hot)
      return;
    TRACE_LOG("_NET_WM_FULLSCREEN_MONITORS h=%lx [%u,%u,%u,%u]", h,
    ...
    if (hot->layer == LAYER_FULLSCREEN) {
      xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, hot->xid,
                          atoms._NET_WM_FULLSCREEN_MONITORS, XCB_ATOM_CARDINAL,
                          32, 4, hot->fullscreen_monitors);
    }
    return;
  }
  ...
  if (ev->type == atoms._NET_REQUEST_FRAME_EXTENTS) {
    ...
    if (h != HANDLE_INVALID) {
      ...
      TRACE_LOG("_NET_REQUEST_FRAME_EXTENTS win=%u undecorated=%d (managed)",
                target, undecorated);
    ...
    // later
    xcb_get_property_cookie_t ck = xcb_get_property(
        s->conn, 0, target, atoms._MOTIF_WM_HINTS, XCB_ATOM_ANY, 0, 5);
    cookie_jar_push(&s->cookie_jar, ck.sequence,
                    COOKIE_GET_PROPERTY_FRAME_EXTENTS, HANDLE_INVALID,
                    (uintptr_t)target, s->txn_id, wm_handle_reply);
    TRACE_LOG("_NET_REQUEST_FRAME_EXTENTS win=%u queued async motif hints",
              target);
    return;
  }
  ...
  if (ev->type == atoms._NET_RESTACK_WINDOW) {
    ...
    TRACE_LOG("_NET_RESTACK_WINDOW h=%lx sibling=%u detail=%u", h, sibling_win,
              detail);
    handle_t sibling_h = (sibling_win != XCB_NONE)
                             ? server_get_client_by_window(s, sibling_win)
                             : HANDLE_INVALID;
    client_hot_t *sib =
        (sibling_h != HANDLE_INVALID) ? server_chot(s, sibling_h) : NULL;
    bool have_sibling = (sib != NULL);
    bool same_layer =
        have_sibling && stack_current_layer(hot) == stack_current_layer(sib);
  ...
  if (ev->type == atoms._NET_MOVERESIZE_WINDOW) {
    ...
    TRACE_LOG("_NET_MOVERESIZE_WINDOW win=%u flags=0x%x gravity=%u x=%d y=%d "
              "w=%u h=%u",
    ...
    uint16_t bw = (hot->flags & CLIENT_FLAG_UNDECORATED)
                      ? 0
                      : s->config.theme.border_width;
    uint16_t th = (hot->flags & CLIENT_FLAG_UNDECORATED)
                      ? 0
                      : s->config.theme.title_height;
  ...
  if (ev->type == atoms._NET_WM_MOVERESIZE) {
    ...
    TRACE_LOG("_NET_WM_MOVERESIZE h=%lx dir=%u button=%u source=%u", h,
              direction, ev->data.data32[3], ev->data.data32[4]);
    if (direction == NET_WM_MOVERESIZE_CANCEL) {
      wm_cancel_interaction(s);
      return;
    }

    bool use_pointer_query = (direction == NET_WM_MOVERESIZE_SIZE_KEYBOARD ||
    ...
    if (s->focused_client != h && hot->type != WINDOW_TYPE_DOCK &&
        ...
        hot->type != WINDOW_TYPE_TOOLTIP && hot->type != WINDOW_TYPE_COMBO &&
    ...
    bool is_keyboard = (direction == NET_WM_MOVERESIZE_SIZE_KEYBOARD ||
                        direction == NET_WM_MOVERESIZE_MOVE_KEYBOARD);
    use_pointer_query |= (root_x == -1 || root_y == -1);

    TRACE_LOG("_NET_WM_MOVERESIZE h=%lx root=%d,%d use_query=%d", h, root_x,
              root_y, use_pointer_query);

    if (use_pointer_query) {
      uintptr_t data = (start_move ? 0x100 : 0) | (is_keyboard ? 0x200 : 0) |
                       (uintptr_t)resize_dir;
      xcb_query_pointer_cookie_t ck = xcb_query_pointer(s->conn, s->root);
      cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_QUERY_POINTER, h,
                      data, s->txn_id, wm_handle_reply);
    } else {
...
void wm_place_window(server_t *s, handle_t h) {
  ...
  if (hot->type == WINDOW_TYPE_DOCK || hot->type == WINDOW_TYPE_DESKTOP ||
      hot->type == WINDOW_TYPE_NOTIFICATION || hot->type == WINDOW_TYPE_MENU ||
      hot->type == WINDOW_TYPE_DROPDOWN_MENU ||
      hot->type == WINDOW_TYPE_POPUP_MENU || hot->type == WINDOW_TYPE_TOOLTIP ||
      hot->type == WINDOW_TYPE_COMBO || hot->type == WINDOW_TYPE_DND) {
  ...
  if (hot->placement == PLACEMENT_CENTER) {
    hot->desired.x =
        (int16_t)(s->workarea.x + (s->workarea.w - hot->desired.w) / 2);
    hot->desired.y =
        (int16_t)(s->workarea.y + (s->workarea.h - hot->desired.h) / 2);
    return;
  } else if (hot->placement == PLACEMENT_MOUSE) {
    xcb_query_pointer_cookie_t cookie = xcb_query_pointer(s->conn, s->root);
    xcb_query_pointer_reply_t *reply =
        xcb_query_pointer_reply(s->conn, cookie, NULL);
    if (reply) {
    ...
    if (parent) {
      hot->desired.x =
          (int16_t)(parent->server.x + (parent->server.w - hot->desired.w) / 2);
      hot->desired.y =
          (int16_t)(parent->server.y + (parent->server.h - hot->desired.h) / 2);
      return;
    }
  }
  ...
  if ((int32_t)hot->desired.x + (int32_t)hot->desired.w >
      (int32_t)s->workarea.x + (int32_t)s->workarea.w) {
    hot->desired.x = (int16_t)(s->workarea.x + s->workarea.w - hot->desired.w);
  }
  if ((int32_t)hot->desired.y + (int32_t)hot->desired.h >
      (int32_t)s->workarea.y + (int32_t)s->workarea.h) {
...
void wm_client_refresh_title(server_t *s, handle_t h) {
  ...
  if (cold->has_net_wm_name && cold->title && cold->title[0] != '\0') {
    xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, hot->xid,
                        atoms._NET_WM_VISIBLE_NAME, atoms.UTF8_STRING, 8,
                        (uint32_t)strlen(cold->title), cold->title);
  } else {
    xcb_delete_property(s->conn, hot->xid, atoms._NET_WM_VISIBLE_NAME);
  }

  if (cold->has_net_wm_icon_name && cold->base_icon_name &&
      cold->base_icon_name[0] != '\0') {
    xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, hot->xid,
                        atoms._NET_WM_VISIBLE_ICON_NAME, atoms.UTF8_STRING, 8,
                        (uint32_t)strlen(cold->base_icon_name),
                        cold->base_icon_name);
  } else {
...
void wm_client_iconify(server_t *s, handle_t h) {
  ...
  TRACE_LOG("iconify h=%lx xid=%u frame=%u layer=%d", h, hot->xid, hot->frame,
            hot->layer);
  ...
  if (s->focused_client == h) {
    ...
    for (list_node_t *node = s->focus_history.next; node != &s->focus_history;
         node = node->next) {
      client_hot_t *cand =
          (client_hot_t *)((char *)node - offsetof(client_hot_t, focus_node));
  ...
  xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, hot->xid, atoms.WM_STATE,
                      atoms.WM_STATE, 32, 2, state_vals);
...
void wm_client_restore(server_t *s, handle_t h) {
  ...
  if (hot->state != STATE_UNMAPPED) {
    LOG_WARN("wm_client_restore called for client %u in state %d (not "
             "UNMAPPED), ignoring",
             hot->xid, hot->state);
    return;
  }
  ...
  TRACE_LOG("restore h=%lx xid=%u frame=%u layer=%d", h, hot->xid, hot->frame,
            hot->layer);
