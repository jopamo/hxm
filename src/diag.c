...
void diag_dump_focus_history(const server_t *s, const char *tag) {
  ...
  LOG_DEBUG("focus_history %s head=%p next=%p prev=%p", tag, (void *)head,
  ...
  while (node != head && guard < 128) {
    ...
    const client_hot_t *c =
        (const client_hot_t *)((const char *)node -
                               offsetof(client_hot_t, focus_node));
    (void)c;
    LOG_DEBUG("  [%d] node=%p prev=%p next=%p h=%lx xid=%u state=%d", guard,
              (void *)node, (void *)node->prev, (void *)node->next, c->self,
              c->xid, c->state);
    node = node->next;
    guard++;
  }
...
void diag_dump_transients(const client_hot_t *hot, const char *tag) {
  ...
  LOG_DEBUG("transients %s h=%lx head=%p next=%p prev=%p", tag, hot->self,
