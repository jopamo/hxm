...
void wm_setup_keys(server_t *s) {
  ...
  for (size_t i = 0; i < s->config.key_bindings.length; i++) {
    ...
    xcb_keycode_t *keycodes =
        xcb_key_symbols_get_keycode(s->keysyms, b->keysym);
    if (!keycodes)
      continue;
    ...
    for (xcb_keycode_t *k = keycodes; *k; k++) {
      ...
      for (size_t m = 0; m < sizeof(IGNORED_MODS) / sizeof(IGNORED_MODS[0]);
           m++) {
        xcb_grab_key(s->conn, 1, s->root, b->modifiers | IGNORED_MODS[m], *k,
...
void wm_handle_key_press(server_t *s, xcb_key_press_event_t *ev) {
  ...
  LOG_DEBUG("Key press: detail=%u state=%u clean=%u sym=%x", ev->detail,
            ev->state, clean_state, sym);
  ...
  for (size_t i = 0; i < s->config.key_bindings.length; i++) {
    ...
    case ACTION_MOVE_TO_WORKSPACE:
      if (b->exec_cmd && s->focused_client != HANDLE_INVALID) {
        wm_client_move_to_workspace(s, s->focused_client,
                                    (uint32_t)safe_atoi(b->exec_cmd), false);
      }
      break;
    ...
    case ACTION_MOVE_TO_WORKSPACE_FOLLOW:
      if (b->exec_cmd && s->focused_client != HANDLE_INVALID) {
        wm_client_move_to_workspace(s, s->focused_client,
                                    (uint32_t)safe_atoi(b->exec_cmd), true);
      }
      break;
    ...
    case ACTION_RESIZE:
      if (s->focused_client != HANDLE_INVALID) {
        ...
        if (b->action == ACTION_MOVE) {
          ...
          cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_QUERY_POINTER,
                          s->focused_client, 0x100, s->txn_id, wm_handle_reply);
        } else {
          // RESIZE: Warp to bottom right
          root_x = hot->server.x + hot->server.w;
          root_y = hot->server.y + hot->server.h;

          xcb_warp_pointer(s->conn, XCB_NONE, s->root, 0, 0, 0, 0, root_x,
                           root_y);

          wm_start_interaction(s, s->focused_client, hot, false,
                               RESIZE_BOTTOM | RESIZE_RIGHT, root_x, root_y,
                               XCB_CURRENT_TIME, true);
        }
      }
      break;
