...
static void draw_button(cairo_t *cr, int x, int y, int w, int h,
                        const char *type, rgba_t color) {
 ...
 */
void render_frame(xcb_connection_t *conn, xcb_window_t win,
                  xcb_visualtype_t *visual, render_context_t *ctx, int depth,
                  bool is_test, const char *title, bool active, int w, int h,
  ...
  if (is_test) {
    ...
    target_surface = cairo_image_surface_create(
        (depth == 32) ? CAIRO_FORMAT_ARGB32 : CAIRO_FORMAT_RGB24, w, h);
  } else {
    if (!visual)
      return; // Safety check
    ...
    xcb_create_pixmap(conn, (uint8_t)depth, pixmap, win, (uint16_t)w,
                      (uint16_t)h);
  ...
  if (dirty && dirty->valid) {
    ...
    if (clip.w > 0 && clip.h > 0) {
      ...
      cairo_rectangle(cr, (double)clip.x, (double)clip.y, (double)clip.w,
                      (double)clip.h);
      cairo_clip(cr);
      use_clip = true;
    }
  ...
  // Map State
  appearance_t *title_bg =
      active ? &theme->window_active_title : &theme->window_inactive_title;
  uint32_t border_color_u32 = active ? theme->window_active_border_color
                                     : theme->window_inactive_border_color;
  uint32_t text_color_u32 = active ? theme->window_active_label_text_color
                                   : theme->window_inactive_label_text_color;
  appearance_t *handle_bg =
      active ? &theme->window_active_handle : &theme->window_inactive_handle;
  appearance_t *grip_bg =
      active ? &theme->window_active_grip : &theme->window_inactive_grip;
  ...
  if (border_w > 0 && border_w * 2 < w && border_w * 2 < h) {
    ...
    cairo_rectangle(cr, (double)border_w / 2.0, (double)border_w / 2.0,
                    (double)w - border_w, (double)h - border_w);
    cairo_stroke(cr);
  }
  ...
  if (is_test) {
    ...
    xcb_put_image(
        conn, XCB_IMAGE_FORMAT_Z_PIXMAP, win, gc, (uint16_t)w, (uint16_t)h, 0,
        0, 0, (uint8_t)depth,
    ...
    xcb_create_gc(conn, gc, win, mask, values);
    if (use_clip) {
      xcb_copy_area(conn, pixmap, win, gc, clip.x, clip.y, clip.x, clip.y,
                    clip.w, clip.h);
    } else {
      xcb_copy_area(conn, pixmap, win, gc, 0, 0, 0, 0, (uint16_t)w,
                    (uint16_t)h);
    }
    xcb_free_gc(conn, gc);
    xcb_free_pixmap(conn, pixmap);
