...
void render_frame(xcb_connection_t* conn, xcb_window_t win, xcb_visualtype_t* visual, render_context_t* ctx, int depth,
    ...
    if (is_test) {
        // Use image surface for tests to avoid XCB-Cairo dependency on dummy connection
    ...
    // We need handle_height and handle_bg. For now we use some defaults or pass them.
    // Let's assume title_bg is passed for now, but we should pass handle_bg too.
    // Actually, I should probably update render_frame signature again to be more complete.
