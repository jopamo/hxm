...
snap_candidate_t snap_compute_candidate(int px, int py, rect_t wa,
                                        int threshold_px) {
  ...
  int right_edge = (right64 > INT32_MAX)
                       ? INT32_MAX
                       : (right64 < INT32_MIN ? INT32_MIN : (int)right64);
  ...
  if (px >= right_edge - thr) {
    ...
    out.rect.y = wa.y;
    out.rect.w =
        (uint16_t)((int)wa.w - half); // right gets the remainder when odd
    out.rect.h = wa.h;
    return out;
  }
