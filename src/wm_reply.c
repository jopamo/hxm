...
static bool client_apply_motif_hints(server_t *s, handle_t h,
  ...
  if (r && r->format == 32 && len >= (int)(3 * sizeof(uint32_t))) {
    const motif_wm_hints_t *mh =
        (const motif_wm_hints_t *)xcb_get_property_value(r);
  ...
  bool changed = (hot->motif_decorations_set != decorations_set ||
...
static bool client_apply_gtk_frame_extents(server_t *s, handle_t h,
                                           const xcb_get_property_reply_t *r) {
  client_hot_t *hot = server_chot(s, h);
  if (!hot)
    return false;
  ...
  int len = r ? xcb_get_property_value_length(r) : 0;
  bool has_extents = (r && r->type == XCB_ATOM_CARDINAL && r->format == 32 &&
                      len >= (int)(4 * sizeof(uint32_t)));
  ...
  if (has_extents) {
    ...
    if (hot->gtk_extents.left != 0 || hot->gtk_extents.right != 0 ||
        hot->gtk_extents.top != 0 || hot->gtk_extents.bottom != 0) {
...
static void client_set_colormap_windows(client_cold_t *cold,
                                        const xcb_window_t *wins,
                                        uint32_t count) {
...
static uint32_t *prop_get_u32_array(const xcb_get_property_reply_t *r,
                                    int min_count, int *out_count) {
  if (!r || r->format != 32)
    return NULL;
  int len = xcb_get_property_value_length(r);
...
static void parse_net_wm_name_like(server_t *s, handle_t h, client_hot_t *hot,
                                   client_cold_t *cold, xcb_atom_t atom,
                                   const xcb_get_property_reply_t *r) {
  int len = 0;
  ...
  if (atom == atoms._NET_WM_NAME) {
    if (!valid) {
      ...
      if (hot->manage_phase != MANAGE_DONE)
        hot->pending_replies++;
      uint32_t c = xcb_get_property(s->conn, 0, hot->xid, atoms.WM_NAME,
    ...
    if (!cold->has_net_wm_name || !cold->base_title ||
        strlen(cold->base_title) != trimmed_len ||
  ...
  if (atom == atoms._NET_WM_ICON_NAME) {
    if (!valid) {
      ...
      if (hot->manage_phase != MANAGE_DONE)
        hot->pending_replies++;
      uint32_t c = xcb_get_property(s->conn, 0, hot->xid, atoms.WM_ICON_NAME,
                                    XCB_ATOM_STRING, 0, 1024)
                       .sequence;
      cookie_jar_push(&s->cookie_jar, c, COOKIE_GET_PROPERTY, h,
                      ((uint64_t)hot->xid << 32) | atoms.WM_ICON_NAME,
                      s->txn_id, wm_handle_reply);
      return;
    }

    if (!cold->has_net_wm_icon_name || !cold->base_icon_name ||
        strlen(cold->base_icon_name) != trimmed_len ||
        strncmp(cold->base_icon_name, str, trimmed_len) != 0) {
      cold->base_icon_name =
          arena_strndup(&cold->string_arena, str, trimmed_len);
...
static void parse_wm_class(client_cold_t *cold,
                           const xcb_get_property_reply_t *r) {
  int len = 0;
  char *str = prop_get_string(r, &len);
  if (!str || len <= 0)
...
static bool client_apply_default_type(server_t *s, client_hot_t *hot,
                                      client_cold_t *cold) {
  (void)s;
  if (!hot || !cold)
    return false;
  ...
  bool changed = hot->type != prev_type || hot->base_layer != prev_base ||
                 hot->placement != prev_place;
...
static bool check_transient_cycle(server_t *s, handle_t child,
                                  handle_t parent) {
  if (child == parent)
    return true;
 ...
 * done.
 */
void wm_handle_reply(server_t *s, const cookie_slot_t *slot, void *reply,
                     xcb_generic_error_t *err) {
  if (err) {
    LOG_DEBUG("Cookie %u returned error code %d", slot->sequence,
              err->error_code);
    reply = NULL;
  }

  if (slot->client == HANDLE_INVALID) {
    if ((slot->type == COOKIE_GET_WINDOW_ATTRIBUTES ||
         slot->type == COOKIE_CHECK_MANAGE_MAP_REQUEST) &&
        reply) {
...
#if HXM_DIAG
      const char *class_str = (r->_class == XCB_WINDOW_CLASS_INPUT_ONLY)
                                  ? "InputOnly"
                                  : "InputOutput";
      (void)class_str;
#endif
      LOG_DEBUG("Classify win=%u override=%d class=%s map_state=%d", win,
                r->override_redirect, class_str, r->map_state);
      ...
      TRACE_LOG("_NET_REQUEST_FRAME_EXTENTS win=%u undecorated=%d (async)", win,
                undecorated);
  ...
  if (slot->type != COOKIE_SYNC_QUERY_COUNTER) {
    if (slot->txn_id < hot->last_applied_txn_id) {
      LOG_DEBUG("Discarding stale reply for client %u (txn_id %lu < last %lu)",
                hot->xid, slot->txn_id, hot->last_applied_txn_id);
      goto done_one;
    }
  ...
  case COOKIE_GET_PROPERTY: {
    ...
    if (atom == atoms.WM_CLASS) {
      ...
      char *str = prop_get_string(r, &len);
      if (str) {
        cold->wm_client_machine =
            arena_strndup(&cold->string_arena, str, (size_t)len);
      }
      ...
      if (!prop_is_empty(r) && r->format == 32 && r->type == XCB_ATOM_WINDOW) {
        ...
        if (bytes >= 4) {
          ...
          client_set_colormap_windows(
              cold, (xcb_window_t *)xcb_get_property_value(r), count);
        } else {
      ...
      if (prop_is_empty(r)) {
        ...
        int num_states = 0;
        xcb_atom_t *states =
            (xcb_atom_t *)prop_get_u32_array(r, 1, &num_states);
        if (states) {
          client_state_set_t set = {0};
      ...
      if (valid) {
        bool hints_changed =
            (hot->hints_flags != next_flags ||
             memcmp(&hot->hints, &next_hints, sizeof(size_hints_t)) != 0);
        ...
        if (hints_changed) {
          ...
          bool is_panel = (hot->type == WINDOW_TYPE_DOCK ||
          ...
          if (hot->state == STATE_NEW && hot->manage_phase != MANAGE_DONE) {
            ...
            if (user_size || prog_size) {
              if (!hot->geometry_from_configure) {
                ...
                if (hot->desired.w == 0 && hints.width > 0 &&
                    (user_size || hints.width > 1))
                  hot->desired.w = (uint16_t)hints.width;

                if (hot->desired.h == 0 && hints.height > 0 &&
                    (user_size || hints.height > 1))
                  hot->desired.h = (uint16_t)hints.height;
              }
            }
            ...
            if (!is_panel) {
              client_constrain_size(&hot->hints, hot->hints_flags,
                                    &hot->desired.w, &hot->desired.h);
            }

          } else if (s->interaction_mode == INTERACTION_RESIZE &&
      ...
      if (xcb_get_property_value_length(r) >= 4) {
        xcb_window_t transient_for_xid =
            *(xcb_window_t *)xcb_get_property_value(r);
      ...
      if (xcb_get_property_value_length(r) > 0) {
        ...
        int num_types =
            xcb_get_property_value_length(r) / (int)sizeof(xcb_atom_t);
      ...
      xcb_atom_t *protocols =
          (xcb_atom_t *)prop_get_u32_array(r, 1, &num_protocols);
      ...
      if (val) {
        ...
        if (hot->sticky == sticky && hot->desktop == new_desk) {
          ...
          wm_client_move_to_workspace(s, slot->client,
                                      sticky ? 0xFFFFFFFFu : desk, false);
        } else {
          hot->sticky = sticky;
    ...
    } else if (atom == atoms._NET_WM_STRUT ||
               atom == atoms._NET_WM_STRUT_PARTIAL) {
      int len = r ? xcb_get_property_value_length(r) : 0;
      bool is_partial = (atom == atoms._NET_WM_STRUT_PARTIAL);
      strut_t *target = is_partial ? &cold->strut_partial : &cold->strut_full;
      bool *active =
          is_partial ? &cold->strut_partial_active : &cold->strut_full_active;
      ...
      if (is_partial && !*active) {
        xcb_get_property_cookie_t ck = xcb_get_property(
            s->conn, 0, hot->xid, atoms._NET_WM_STRUT, XCB_ATOM_CARDINAL, 0, 4);
        cookie_jar_push(&s->cookie_jar, ck.sequence, COOKIE_GET_PROPERTY,
                        slot->client,
      ...
      if (prop_is_empty(r)) {
        bool changed_any = (cold->can_focus != true ||
        ...
        if (xcb_icccm_get_wm_hints_from_reply(&hints, r)) {
          ...
          if (cold->can_focus != next_can_focus ||
      ...
      if (!prop_is_empty(r)) {
        ...
        while (i + 2 <= total_words) {
          ...
          if (w <= icon_dim_max && h <= icon_dim_max) {
            ...
            for (size_t t = 0;
                 t < sizeof(icon_target_sizes) / sizeof(icon_target_sizes[0]);
                 t++) {
        ...
        if (best_data) {
          if (hot->icon_surface)
            cairo_surface_destroy(hot->icon_surface);
          hot->icon_surface = cairo_image_surface_create(
              CAIRO_FORMAT_ARGB32, (int)best_w, (int)best_h);
      ...
      if (prop_is_cardinal(r) && xcb_get_property_value_length(r) >= 4) {
        xcb_sync_counter_t counter =
            *(xcb_sync_counter_t *)xcb_get_property_value(r);
        hot->sync_counter = counter;
        hot->sync_value = 0;
        if (counter != XCB_NONE) {
          xcb_sync_query_counter_cookie_t ck =
              xcb_sync_query_counter(s->conn, counter);
          cookie_jar_push(&s->cookie_jar, ck.sequence,
                          COOKIE_SYNC_QUERY_COUNTER, slot->client,
        ...
        if (!hot->window_opacity_valid || hot->window_opacity != val) {
          ...
          if (hot->frame != XCB_NONE) {
            xcb_change_property(s->conn, XCB_PROP_MODE_REPLACE, hot->frame,
                                atoms._NET_WM_WINDOW_OPACITY, XCB_ATOM_CARDINAL,
                                32, 1, &val);
          }
        }
      } else {
      ...
      if (prop_is_cardinal(r) && xcb_get_property_value_length(r) >= 16) {
        uint32_t *val = (uint32_t *)xcb_get_property_value(r);
        rect_t next_geom = {(int16_t)val[0], (int16_t)val[1], (uint16_t)val[2],
                            (uint16_t)val[3]};
        if (!hot->icon_geometry_valid ||
            memcmp(&hot->icon_geometry, &next_geom, sizeof(rect_t)) != 0) {
  ...
  case COOKIE_QUERY_POINTER: {
    ...
    if (!is_keyboard &&
        !(r->mask & (XCB_KEY_BUT_MASK_BUTTON_1 | XCB_KEY_BUT_MASK_BUTTON_2 |
    ...
    wm_start_interaction(s, slot->client, hot, is_move, resize_dir, root_x,
                         root_y, 0, is_keyboard);
    break;
  }
  ...
  case COOKIE_SYNC_QUERY_COUNTER: {
    ...
    if (hot->sync_counter == counter) {
      uint64_t uvalue = ((uint64_t)(uint32_t)r->counter_value.hi << 32) |
                        (uint64_t)(uint32_t)r->counter_value.lo;
