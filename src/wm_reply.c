...
static void client_apply_motif_hints(server_t *s, handle_t h,
...
static void client_apply_gtk_frame_extents(server_t *s, handle_t h,
                                           const xcb_get_property_reply_t *r) {
  client_hot_t *hot = server_chot(s, h);
  if (!hot)
    return;
  ...
  int len = r ? xcb_get_property_value_length(r) : 0;
  bool has_extents =
      (r && r->format == 32 && len >= (int)(4 * sizeof(uint32_t)));
...
void wm_handle_reply(server_t *s, const cookie_slot_t *slot, void *reply,
  ...
  case COOKIE_GET_PROPERTY: {
    ...
    if (atom == atoms.WM_CLASS) {
      ...
      TRACE_LOG("strut_reply xid=%u atom=%s len=%d active=%d top=%u", hot->xid,
                is_partial ? "_NET_WM_STRUT_PARTIAL" : "_NET_WM_STRUT", len,
  ...
  case COOKIE_SYNC_QUERY_COUNTER: {
    ...
    if (hot->sync_counter == counter) {
      int64_t value =
          ((int64_t)r->counter_value.hi << 32) | r->counter_value.lo;
      if (value < 0)
        value = 0;
