...
static void add_keybind(config_t *config, uint32_t mods, xcb_keysym_t sym,
                        action_type_t action, const char *cmd) {
  key_binding_t *b = calloc(1, sizeof(*b));
  b->modifiers = mods;
  b->keysym = sym;
  b->action = action;
  if (cmd)
    b->exec_cmd = strdup(cmd);
...
void config_init_defaults(config_t *config) {
  ...
  add_keybind(config, XCB_MOD_MASK_1 | XCB_MOD_MASK_SHIFT, XK_Tab,
              ACTION_FOCUS_PREV, NULL);
  ...
  for (int i = 0; i < 9; i++) {
    ...
    add_keybind(config, XCB_MOD_MASK_4 | XCB_MOD_MASK_SHIFT, XK_1 + i,
                ACTION_MOVE_TO_WORKSPACE, buf);
  }
...
void config_destroy(config_t *config) {
  ...
  if (config->desktop_names) {
    for (uint32_t i = 0; i < config->desktop_names_count; i++) {
      if (config->desktop_names[i])
        free(config->desktop_names[i]);
    }
...
static background_style_t parse_appearance_flags(const char *val) {
  // Flags are space-separated words like "solid flat" or "gradient vertical
  // raised"
...
static void parse_keybind(config_t *config, const char *val) {
  ...
  // Left side is modifiers + keysym, right side is action [and optional
  // argument]
  char *copy = strdup(val);
...
static void parse_rule(config_t *config, const char *val) {
  // Format: "class:Firefox, title:Preferences -> desktop:2, layer:above,
  // focus:true" Match side and action side are comma-separated key:value pairs
  char *copy = strdup(val);
    ...
    if (sep) {
      *sep = '\0';
      ...
      if (strcasecmp(k, "desktop") == 0) {
        ...
        r->focus = (strcasecmp(v, "yes") == 0 || strcasecmp(v, "true") == 0 ||
                    strcmp(v, "1") == 0);
    ...
    if (strcmp(key, "active_bg") == 0) {
      ...
      if (config->desktop_names) {
        for (uint32_t j = 0; j < config->desktop_names_count; j++) {
          if (config->desktop_names[j])
            free(config->desktop_names[j]);
        }
        free(config->desktop_names);
        config->desktop_names = NULL;
        config->desktop_names_count = 0;
      ...
      config->fullscreen_use_workarea =
          (strcasecmp(val, "true") == 0 || strcmp(val, "1") == 0);
