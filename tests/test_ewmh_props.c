...
static bool atom_in_state_list(const xcb_atom_t *atoms_list, uint32_t count,
                               xcb_atom_t needle);

static const struct stub_prop_call *
find_prop_call(xcb_window_t win, xcb_atom_t atom, bool deleted) {
...
static handle_t add_mapped_client(server_t *s, xcb_window_t win,
                                  xcb_window_t frame) {
...
static void test_active_window_updates(void) {
  ...
  const struct stub_prop_call *call =
      find_prop_call(s.root, atoms._NET_ACTIVE_WINDOW, false);
  assert(call != NULL);
  assert(call->len == 1);
  ...
  const struct stub_prop_call *del =
      find_prop_call(s.root, atoms._NET_ACTIVE_WINDOW, true);
  assert(del != NULL);
...
static void test_client_list_add_remove(void) {
  ...
  const struct stub_prop_call *list =
      find_prop_call(s.root, atoms._NET_CLIENT_LIST, false);
  assert(list != NULL);
  assert(list->len == 2);
  ...
  const struct stub_prop_call *list2 =
      find_prop_call(s.root, atoms._NET_CLIENT_LIST, false);
  assert(list2 != NULL);
  assert(list2->len == 1);
...
static void test_client_list_includes_all_managed(void) {
  ...
  const struct stub_prop_call *list_stack =
      find_prop_call(s.root, atoms._NET_CLIENT_LIST_STACKING, false);
...
static void test_desktop_props_publish_and_switch(void) {
  ...
  const struct stub_prop_call *num =
      find_prop_call(s.root, atoms._NET_NUMBER_OF_DESKTOPS, false);
  assert(num != NULL);
  const uint32_t *num_vals = (const uint32_t *)num->data;
  assert(num_vals[0] == 3);

  const struct stub_prop_call *cur =
      find_prop_call(s.root, atoms._NET_CURRENT_DESKTOP, false);
  assert(cur != NULL);
  ...
  const struct stub_prop_call *cur2 =
      find_prop_call(s.root, atoms._NET_CURRENT_DESKTOP, false);
  assert(cur2 != NULL);
...
static void test_net_wm_desktop_reply_moves_after_manage(void) {
  ...
  const struct stub_prop_call *desk =
      find_prop_call(hot->xid, atoms._NET_WM_DESKTOP, false);
  assert(desk != NULL);
...
static void test_strut_updates_workarea(void) {
  ...
  const struct stub_prop_call *wa =
      find_prop_call(s.root, atoms._NET_WORKAREA, false);
  assert(wa != NULL);
  ...
  const struct stub_prop_call *wa2 =
      find_prop_call(s.root, atoms._NET_WORKAREA, false);
  assert(wa2 != NULL);
...
static void test_state_below_sticky_skip_applies(void) {
  ...
  const struct stub_prop_call *state =
      find_prop_call(hot->xid, atoms._NET_WM_STATE, false);
  assert(state != NULL);
  assert(atom_in_state_list((xcb_atom_t *)state->data, state->len,
