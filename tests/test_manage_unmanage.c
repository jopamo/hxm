...
extern void xcb_stubs_set_query_tree_children(const xcb_window_t *children,
                                              int len);
extern bool xcb_stubs_attr_request_window(uint32_t seq,
...
static int adopt_poll_for_reply(xcb_connection_t *c, unsigned int request,
  ...
  if (!xcb_stubs_attr_request_window(request, &win))
    return 0;
...
static int map_request_poll_for_reply(xcb_connection_t *c, unsigned int request,
                                      void **reply,
                                      xcb_generic_error_t **error) {
  (void)c;
  if (error)
...
static void test_adopt_children_skips_override_and_unmapped(void) {
  ...
  g_adopt_attrs_len = 0;
  g_adopt_attrs[g_adopt_attrs_len++] =
      (adopt_attr_t){w1, false, XCB_MAP_STATE_VIEWABLE};
  g_adopt_attrs[g_adopt_attrs_len++] =
      (adopt_attr_t){w2, true, XCB_MAP_STATE_VIEWABLE};
  g_adopt_attrs[g_adopt_attrs_len++] =
      (adopt_attr_t){w3, false, XCB_MAP_STATE_UNMAPPED};
...
static bool cookie_jar_has_atom(cookie_jar_t *cj, xcb_atom_t atom) {
  ...
  for (size_t i = 0; i < cj->cap; i++) {
    ...
    if (!slot->live)
      continue;
    if (slot->type != COOKIE_GET_PROPERTY)
      continue;
...
static void test_reparent_notify_ignored(void) {
  ...
  xcb_stubs_reset();
  xcb_stubs_enqueue_event(
      (xcb_generic_event_t *)memcpy(calloc(1, sizeof(ev)), &ev, sizeof(ev)));
  event_ingest(&s, true);
