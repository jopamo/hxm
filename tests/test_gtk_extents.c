...
static void test_gtk_extents_inflation_order_and_state(void) {
  ...
  // Even if we have border width, GTK extents should dominate for undecorated
  // client
  ...
  // Frame: Positioned at desired (logical) pos MINUS extents, Sized to content
  // (includes shadow)
...
static void test_no_gtk_extents_no_inflation(void) {
  ...
  const uint32_t exp_frame_w = 400 + 2 * bw;
  const uint32_t exp_frame_h = 300 + ts.s.config.theme.title_height + bottom;
  const uint32_t exp_client_w = 400;
  const uint32_t exp_client_h = 300;
