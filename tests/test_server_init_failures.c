...
// We need to provide dummy implementations for close() because server_cleanup
// calls it and we are returning fake FDs (100, 101, 102). Closing them might be
// harmless or error. But we are not wrapping close(), so it calls real close.
...
void set_fail_connect(void) {
  fail_connect = true;
}
void set_fail_fcntl(void) {
  fail_fcntl = true;
}
void set_fail_keysyms(void) {
  fail_keysyms = true;
}
void set_fail_sigprocmask(void) {
  fail_sigprocmask = true;
}
void set_fail_signalfd(void) {
  fail_signalfd = true;
}
void set_fail_timerfd(void) {
  fail_timerfd = true;
}
void set_fail_epoll_create(void) {
  fail_epoll_create = true;
}
void set_fail_epoll_ctl(void) {
  fail_epoll_ctl = true;
}
