...
int main(void) {
  ...
  xcb_create_window(conn, XCB_COPY_FROM_PARENT, win, screen->root, 40, 40, 200,
                    120, 1, XCB_WINDOW_CLASS_INPUT_OUTPUT, screen->root_visual,
                    mask, values);
  ...
  if (wm_class != XCB_ATOM_NONE) {
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, win, wm_class,
                        XCB_ATOM_STRING, 8, (uint32_t)sizeof(class_data),
                        class_data);
  }

  const char name_data[] = "hxm-normal";
  if (wm_name != XCB_ATOM_NONE) {
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, win, wm_name,
                        XCB_ATOM_STRING, 8, (uint32_t)(sizeof(name_data) - 1),
                        name_data);
  }
  if (net_wm_name != XCB_ATOM_NONE && utf8 != XCB_ATOM_NONE) {
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, win, net_wm_name, utf8, 8,
                        (uint32_t)(sizeof(name_data) - 1), name_data);
  }
