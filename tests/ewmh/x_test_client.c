...
static bool has_extension(xcb_connection_t *conn, const char *name) {
  xcb_query_extension_cookie_t cookie =
      xcb_query_extension(conn, (uint16_t)strlen(name), name);
...
int main(int argc, char **argv) {
  ...
  if (strcmp(cmd, "create-window") == 0) {
    ...
    xcb_create_window(conn, XCB_COPY_FROM_PARENT, win, root, 0, 0, 100, 100, 1,
  ...
  if (strcmp(cmd, "get-root-cardinals") == 0) {
    ...
    xcb_get_property_cookie_t ck =
        xcb_get_property(conn, 0, root, atom, XCB_ATOM_ANY, 0, UINT32_MAX);
  ...
  if (strcmp(cmd, "get-window-cardinals") == 0) {
    ...
    xcb_get_property_cookie_t ck =
        xcb_get_property(conn, 0, win, atom, XCB_ATOM_ANY, 0, UINT32_MAX);
  ...
  if (strcmp(cmd, "set-window-cardinals") == 0) {
    ...
    xcb_void_cookie_t ck =
        xcb_change_property_checked(conn, XCB_PROP_MODE_REPLACE, win, atom,
                                    XCB_ATOM_CARDINAL, 32, count, vals);
    xcb_generic_error_t *err = xcb_request_check(conn, ck);
    if (err) {
      fprintf(stderr,
              "x_test_client: change_property failed (cardinals) err=%d\n",
              err->error_code);
      free(err);
      free(vals);
      xcb_disconnect(conn);
  ...
  if (strcmp(cmd, "set-window-atoms") == 0) {
    ...
    xcb_void_cookie_t ck = xcb_change_property_checked(
        conn, XCB_PROP_MODE_REPLACE, win, atom, XCB_ATOM_ATOM, 32, count, vals);
    xcb_generic_error_t *err = xcb_request_check(conn, ck);
    if (err) {
  ...
  if (strcmp(cmd, "set-window-string") == 0) {
    ...
    xcb_void_cookie_t ck = xcb_change_property_checked(
        conn, XCB_PROP_MODE_REPLACE, win, atom, type, 8, len, value);
    xcb_generic_error_t *err = xcb_request_check(conn, ck);
    if (err) {
  ...
  if (strcmp(cmd, "set-window-empty") == 0) {
    ...
    xcb_atom_t atom = get_atom(conn, argv[3]);
    xcb_atom_t type = get_atom(conn, argv[4]);
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, win, atom, type, 32, 0,
                        NULL);
    xcb_flush(conn);
    xcb_disconnect(conn);
    return 0;
  ...
  if (strcmp(cmd, "assert-substructure-redirect") == 0) {
    ...
    if (!err) {
      fprintf(
          stderr,
          "x_test_client: SubstructureRedirect available (WM not running)\n");
      xcb_disconnect(conn);
      return 1;
    }
