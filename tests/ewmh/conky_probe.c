...
static bool property_present(xcb_connection_t *conn, xcb_window_t win,
                             xcb_atom_t prop, bool require_nonempty) {
  xcb_get_property_cookie_t ck =
      xcb_get_property(conn, 0, win, prop, XCB_ATOM_ANY, 0, 128);
...
static bool wait_for_property(xcb_connection_t *conn, xcb_window_t win,
                              xcb_atom_t prop, uint32_t timeout_ms,
                              bool require_nonempty) {
  uint32_t waited = 0;
  while (waited <= timeout_ms) {
    if (property_present(conn, win, prop, require_nonempty))
      return true;
    sleep_ms(50);
    waited += 50;
  }
...
static bool fetch_atom_list(xcb_connection_t *conn, xcb_window_t win,
                            xcb_atom_t prop, xcb_atom_t **atoms_out,
                            uint32_t *len_out) {
  xcb_get_property_cookie_t ck =
      xcb_get_property(conn, 0, win, prop, XCB_ATOM_ATOM, 0, 32);
  xcb_get_property_reply_t *rep = xcb_get_property_reply(conn, ck, NULL);
  if (!rep)
    return false;
  ...
  uint32_t len =
      (uint32_t)(xcb_get_property_value_length(rep) / sizeof(xcb_atom_t));
...
static bool atom_list_contains(const xcb_atom_t *atoms, uint32_t len,
                               xcb_atom_t needle) {
...
static char *get_text_property(xcb_connection_t *conn, xcb_window_t win,
                               xcb_atom_t prop) {
  xcb_get_property_cookie_t ck =
      xcb_get_property(conn, 0, win, prop, XCB_ATOM_ANY, 0, 1024);
  xcb_get_property_reply_t *rep = xcb_get_property_reply(conn, ck, NULL);
  if (!rep)
    return NULL;
...
static bool window_matches_class(xcb_connection_t *conn, xcb_window_t win,
                                 xcb_atom_t wm_class, const char *match) {
  if (!match || !*match || wm_class == XCB_ATOM_NONE)
    return false;
  char *data = get_text_property(conn, win, wm_class);
  if (!data)
    return false;
  bool ok = false;
  if (contains_case_insensitive(data, match))
    ok = true;
  free(data);
  return ok;
}

static bool window_matches_name(xcb_connection_t *conn, xcb_window_t win,
                                xcb_atom_t net_wm_name, xcb_atom_t wm_name,
                                const char *match) {
  if (!match || !*match)
    return false;
  char *name = NULL;
  if (net_wm_name != XCB_ATOM_NONE)
    name = get_text_property(conn, win, net_wm_name);
  if (!name && wm_name != XCB_ATOM_NONE)
    name = get_text_property(conn, win, wm_name);
  if (!name)
    return false;
...
static bool wm_state_is_normal(xcb_connection_t *conn, xcb_window_t win,
                               xcb_atom_t wm_state) {
  xcb_get_property_cookie_t ck =
      xcb_get_property(conn, 0, win, wm_state, wm_state, 0, 2);
  xcb_get_property_reply_t *rep = xcb_get_property_reply(conn, ck, NULL);
  if (!rep)
    return false;
  bool ok = false;
...
static bool get_window_geometry_on_root(xcb_connection_t *conn,
                                        xcb_window_t win, xcb_window_t root,
                                        int16_t *rx, int16_t *ry, uint16_t *w,
                                        uint16_t *h) {
  ...
  *h = greply->height;
  ...
  xcb_translate_coordinates_cookie_t tck =
      xcb_translate_coordinates(conn, win, root, 0, 0);
  xcb_translate_coordinates_reply_t *treply =
      xcb_translate_coordinates_reply(conn, tck, NULL);
  if (!treply) {
    free(greply);
    return false;
  }
...
static bool get_root_stacking_list(xcb_connection_t *conn, xcb_window_t root,
                                   xcb_atom_t prop, xcb_window_t **out,
                                   uint32_t *len_out) {
  xcb_get_property_cookie_t ck =
      xcb_get_property(conn, 0, root, prop, XCB_ATOM_WINDOW, 0, 1024);
  xcb_get_property_reply_t *rep = xcb_get_property_reply(conn, ck, NULL);
  if (!rep)
    return false;
  if (rep->type == XCB_ATOM_NONE || rep->format != 32) {
  ...
  uint32_t len =
      (uint32_t)(xcb_get_property_value_length(rep) / sizeof(xcb_window_t));
  if (len == 0) {
    free(rep);
    return false;
  }
...
static size_t append_unique(xcb_window_t *out, size_t count, size_t cap,
                            xcb_window_t win) {
...
static size_t collect_candidate_windows(xcb_connection_t *conn,
                                        xcb_window_t root,
                                        xcb_atom_t stacking_prop,
                                        xcb_window_t *out, size_t cap,
                                        bool *used_stacking) {
  *used_stacking = false;
  xcb_window_t *list = NULL;
  uint32_t len = 0;
  if (stacking_prop != XCB_ATOM_NONE &&
      get_root_stacking_list(conn, root, stacking_prop, &list, &len)) {
    size_t count = 0;
  ...
  for (int i = 0; i < child_len && count < cap; i++) {
    ...
    xcb_query_tree_reply_t *rep_child =
        xcb_query_tree_reply(conn, ck_child, NULL);
    if (rep_child) {
...
static bool wait_for_wm_ready(xcb_connection_t *conn, xcb_window_t root,
                              xcb_atom_t prop, uint32_t timeout_ms) {
  uint32_t waited = 0;
  while (waited <= timeout_ms) {
    xcb_get_property_cookie_t ck =
        xcb_get_property(conn, 0, root, prop, XCB_ATOM_WINDOW, 0, 1);
    xcb_get_property_reply_t *rep = xcb_get_property_reply(conn, ck, NULL);
    if (rep && rep->type == XCB_ATOM_WINDOW && rep->format == 32 &&
        rep->value_len >= 1) {
...
int main(void) {
  ...
  atoms.net_wm_window_type_desktop =
      get_atom(conn, "_NET_WM_WINDOW_TYPE_DESKTOP");
  ...
  atoms.net_wm_state_skip_taskbar =
      get_atom(conn, "_NET_WM_STATE_SKIP_TASKBAR");
  ...
  if (!wait_for_wm_ready(conn, screen->root, atoms.net_supporting_wm_check,
                         (uint32_t)wm_timeout)) {
    die("WM did not become ready in time");
  }

  const char *conky_class_match = getenv_default("CONKY_CLASS_MATCH", "Conky");
  const char *conky_name_match =
      getenv_default("CONKY_NAME_MATCH", "hxm-conky-test");
  const char *normal_class_match =
      getenv_default("NORMAL_CLASS_MATCH", "HxmNormal");
  const char *normal_name_match =
      getenv_default("NORMAL_NAME_MATCH", "hxm-normal");
  ...
  while (waited <= (uint32_t)window_timeout) {
    ...
    size_t count = collect_candidate_windows(conn, screen->root,
                                             atoms.net_client_list_stacking,
                                             candidates, 1024, &used_stacking);
    for (size_t i = 0; i < count; i++) {
      xcb_window_t win = candidates[i];
      if (conky_win == XCB_NONE) {
        if (window_matches_class(conn, win, atoms.wm_class,
                                 conky_class_match) ||
            window_matches_name(conn, win, atoms.net_wm_name, atoms.wm_name,
                                conky_name_match)) {
          conky_win = win;
        }
      }
    ...
    if (conky_win != XCB_NONE && (!check_stacking || normal_win != XCB_NONE))
      break;
    sleep_ms(50);
    waited += 50;
  }
  ...
  if (conky_win == XCB_NONE)
    die("Conky window not found");
  if (check_stacking && normal_win == XCB_NONE)
    die("Normal test window not found for stacking check");

  xcb_window_t *type_atoms = NULL;
  uint32_t type_len = 0;
  if (!fetch_atom_list(conn, conky_win, atoms.net_wm_window_type, &type_atoms,
                       &type_len)) {
  ...
  if (strcasecmp(expect_type, "ANY") == 0) {
    ...
    type_ok = atom_list_contains(type_atoms, type_len,
                                 atoms.net_wm_window_type_desktop);
  } else {
    type_ok =
        atom_list_contains(type_atoms, type_len, atoms.net_wm_window_type_dock);
  }
  free(type_atoms);
  if (!type_ok)
    die("_NET_WM_WINDOW_TYPE mismatch");

  xcb_window_t *state_atoms = NULL;
  uint32_t state_len = 0;
  if (!fetch_atom_list(conn, conky_win, atoms.net_wm_state, &state_atoms,
                       &state_len)) {
    die("_NET_WM_STATE not readable");
  }

  if (expect_below &&
      !atom_list_contains(state_atoms, state_len, atoms.net_wm_state_below)) {
    free(state_atoms);
    die("_NET_WM_STATE missing BELOW");
  }
  if (expect_sticky &&
      !atom_list_contains(state_atoms, state_len, atoms.net_wm_state_sticky)) {
    free(state_atoms);
    die("_NET_WM_STATE missing STICKY");
  }
  if (expect_skip_taskbar &&
      !atom_list_contains(state_atoms, state_len,
                          atoms.net_wm_state_skip_taskbar)) {
    free(state_atoms);
    die("_NET_WM_STATE missing SKIP_TASKBAR");
  }
  if (expect_skip_pager && !atom_list_contains(state_atoms, state_len,
  ...
  if (!wait_for_property(conn, conky_win, atoms.wm_hints,
                         (uint32_t)window_timeout, true)) {
    if (require_hints) {
  ...
  if (!wait_for_property(conn, conky_win, atoms.wm_normal_hints,
                         (uint32_t)window_timeout, true)) {
  ...
  xcb_get_window_attributes_cookie_t ack =
      xcb_get_window_attributes(conn, conky_win);
  xcb_get_window_attributes_reply_t *arep =
      xcb_get_window_attributes_reply(conn, ack, NULL);
  if (!arep)
    die("window attributes failed");
  ...
  int16_t rx = 0, ry = 0;
  uint16_t w = 0, h = 0;
  if (!get_window_geometry_on_root(conn, conky_win, screen->root, &rx, &ry, &w,
                                   &h)) {
  ...
  bool intersects =
      !(sx + sw <= 0 || sy + sh <= 0 || sx >= screen_w || sy >= screen_h);
  if (!intersects) {
  ...
  if (check_stacking) {
    xcb_window_t *list = NULL;
    uint32_t len = 0;
    bool got_list = get_root_stacking_list(
        conn, screen->root, atoms.net_client_list_stacking, &list, &len);
    if (!got_list) {
