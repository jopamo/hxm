...
static bool wait_for_property(xcb_connection_t* conn, xcb_window_t win, xcb_atom_t prop, uint32_t timeout_ms, bool require_nonempty) {
  uint32_t waited = 0;
  while (waited <= timeout_ms) {
...
static bool fetch_atom_list(xcb_connection_t* conn, xcb_window_t win, xcb_atom_t prop, xcb_atom_t** atoms_out, uint32_t* len_out) {
...
static bool window_matches_name(xcb_connection_t* conn, xcb_window_t win, xcb_atom_t net_wm_name, xcb_atom_t wm_name, const char* match) {
  if (!match || !*match)
    return false;
  char* name = NULL;
...
static bool get_window_geometry_on_root(xcb_connection_t* conn, xcb_window_t win, xcb_window_t root, int16_t* rx, int16_t* ry, uint16_t* w, uint16_t* h) {
...
static bool get_root_stacking_list(xcb_connection_t* conn, xcb_window_t root, xcb_atom_t prop, xcb_window_t** out, uint32_t* len_out) {
...
static size_t collect_candidate_windows(xcb_connection_t* conn, xcb_window_t root, xcb_atom_t stacking_prop, xcb_window_t* out, size_t cap, bool* used_stacking) {
  *used_stacking = false;
...
int main(void) {
  ...
  while (waited <= (uint32_t)window_timeout) {
    ...
    size_t count = collect_candidate_windows(conn, screen->root, atoms.net_client_list_stacking, candidates, 1024, &used_stacking);
    for (size_t i = 0; i < count; i++) {
      xcb_window_t win = candidates[i];
      if (conky_win == XCB_NONE) {
        if (window_matches_class(conn, win, atoms.wm_class, conky_class_match) || window_matches_name(conn, win, atoms.net_wm_name, atoms.wm_name, conky_name_match)) {
          conky_win = win;
        }
      }
      if (normal_win == XCB_NONE) {
        if (window_matches_class(conn, win, atoms.wm_class, normal_class_match) || window_matches_name(conn, win, atoms.net_wm_name, atoms.wm_name, normal_name_match)) {
          normal_win = win;
        }
      }
    }
