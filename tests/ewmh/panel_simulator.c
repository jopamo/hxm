...
static uint32_t *get_u32_property_list(xcb_connection_t *conn, xcb_window_t win,
                                       xcb_atom_t prop, xcb_atom_t type,
                                       size_t *out_len) {
  xcb_get_property_cookie_t ck =
      xcb_get_property(conn, 0, win, prop, type, 0, UINT32_MAX);
...
static void print_atom_list(xcb_connection_t *conn, const uint32_t *vals,
                            size_t len) {
  printf("[");
  for (size_t i = 0; i < len; i++) {
    if (i > 0)
...
static void watch_window(panel_ctx_t *ctx, xcb_window_t win) {
  ...
  xcb_void_cookie_t ck = xcb_change_window_attributes_checked(
      ctx->conn, win, XCB_CW_EVENT_MASK, &mask);
...
static void update_root_lists(panel_ctx_t *ctx) {
  size_t len = 0;
  uint32_t *list = get_u32_property_list(
      ctx->conn, ctx->root, ctx->atom_client_list, XCB_ATOM_WINDOW, &len);
  ...
  len = 0;
  list = get_u32_property_list(ctx->conn, ctx->root,
                               ctx->atom_client_list_stacking, XCB_ATOM_WINDOW,
                               &len);
...
static void handle_window_prop(panel_ctx_t *ctx, xcb_window_t win,
  ...
  if (atom == ctx->atom_wm_state) {
    uint32_t *vals =
        get_u32_property_list(ctx->conn, win, atom, XCB_ATOM_ATOM, &len);
    ...
    uint32_t *vals =
        get_u32_property_list(ctx->conn, win, atom, XCB_ATOM_CARDINAL, &len);
...
int main(int argc, char **argv) {
  ...
  ctx.atom_client_list_stacking =
      get_atom(ctx.conn, "_NET_CLIENT_LIST_STACKING");
  ...
  xcb_change_window_attributes(ctx.conn, ctx.root, XCB_CW_EVENT_MASK,
                               &root_mask);
  xcb_flush(ctx.conn);
  ...
  while (duration_sec == 0 || now_us() < end_us) {
    ...
    if (type == XCB_PROPERTY_NOTIFY) {
      ...
      if (p->window == ctx.root && (p->atom == ctx.atom_client_list ||
                                    p->atom == ctx.atom_client_list_stacking)) {
        update_root_lists(&ctx);
      } else if (window_set_contains(&ctx.tracked, p->window) &&
                 (p->atom == ctx.atom_wm_state ||
                  p->atom == ctx.atom_wm_window_type ||
                  p->atom == ctx.atom_wm_desktop)) {
