...
static uint32_t* get_u32_property_list(xcb_connection_t* conn, xcb_window_t win, xcb_atom_t prop, xcb_atom_t type,
                                       size_t* out_len) {
...
static bool window_set_contains(const window_set_t* set, xcb_window_t win) {
    for (size_t i = 0; i < set->len; i++) {
        if (set->items[i] == win) return true;
    }
    return false;
}

static void window_set_add(window_set_t* set, xcb_window_t win) {
    if (window_set_contains(set, win)) return;
    if (set->len == set->cap) {
...
static void watch_window(panel_ctx_t* ctx, xcb_window_t win) {
    if (window_set_contains(&ctx->tracked, win)) return;
...
int main(int argc, char** argv) {
    ...
    if (xcb_connection_has_error(ctx.conn)) die("cannot connect to X server");
    ...
    uint64_t end_us = 0;
    if (duration_sec > 0) end_us = now_us() + (uint64_t)duration_sec * 1000000ull;
    ...
    while (duration_sec == 0 || now_us() < end_us) {
        ...
        if (type == XCB_PROPERTY_NOTIFY) {
            ...
            if (p->window == ctx.root &&
                (p->atom == ctx.atom_client_list || p->atom == ctx.atom_client_list_stacking)) {
                update_root_lists(&ctx);
            } else if (window_set_contains(&ctx.tracked, p->window) &&
                       (p->atom == ctx.atom_wm_state || p->atom == ctx.atom_wm_window_type ||
