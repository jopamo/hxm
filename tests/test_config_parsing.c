...
static void test_defaults(void) {
  ...
  for (size_t i = 0; i < c.key_bindings.length; i++) {
    ...
    if (b->keysym == XK_Return && b->modifiers == XCB_MOD_MASK_4 &&
        b->action == ACTION_TERMINAL) {
      found_term = true;
    }
  }
  assert(found_term);
...
static void test_load_simple(void) {
  const char *content = "desktop_count=6\n"
                        "border_width=5\n"
...
static void test_keybinds(void) {
  const char *content = "clear_keybinds=\n" // Should clear defaults
...
static void test_theme(void) {
  const char *content = "window.active.title.bg: gradient vertical\n"
