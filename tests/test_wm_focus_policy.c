...
static handle_t alloc_test_client(server_t *s, xcb_window_t xid,
                                  int32_t desktop) {
...
void test_focus_on_finish_manage(void) {
  ...
  // 2. Manage second normal window -> should NOT get focus (prevention of
  // stealing)
...
void test_mru_cycling(void) {
  ...
  // Cycle Next (forward) -> should focus h[2] (because h[1] was moved to head,
  // h[2] is now second) Wait, MRU behavior: Start: [2, 1, 0] Cycle Next from 2
  // -> finds 1. wm_set_focus(1) -> [1, 2, 0] Cycle Next from 1 -> finds 2.
  // wm_set_focus(2) -> [2, 1, 0] So it toggles. This is expected without a
  // cycling session.
  wm_cycle_focus(&s, true);
  assert(s.focused_client == h[2]);
  ...
  // Cycle Next from 2 -> skips 1 (DOCK) -> finds 0. wm_set_focus(0) -> [0, 2,
  // 1]
  wm_cycle_focus(&s, true);
  ...
  // Cycle Next from 0 -> skips 2 (wrong desktop) -> skips 1 (DOCK) -> loops
  // back to 0 -> stops Wait, the loop stops when it hits start_node.
  wm_cycle_focus(&s, true);
  assert(s.focused_client == h[0]);
