...
static void test_event_ingest_coalesces_configure_request(void) {
  ...
  assert(pc != NULL);
  assert(pc->mask == (XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y |
...
static void test_event_ingest_coalesces_randr(void) {
  ...
  ev1->response_type =
      (uint8_t)(s.randr_event_base + XCB_RANDR_SCREEN_CHANGE_NOTIFY);
  ev1->width = 800;
  ev1->height = 600;
...
static void test_event_ingest_coalesces_motion_notify(void) {
  ...
  xcb_motion_notify_event_t *final_ev =
      hash_map_get(&s.buckets.motion_notifies, win);
