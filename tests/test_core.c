...
static void test_counters_edge_cases(void) {
  counters_init();
  // Simulate a case where tick_count > 0 but min is still sentinel (e.g. manual
  // manipulation or bug) This exercises the 'if (min == UINT64_MAX) min = 0'
  // branch in print_tick_stats
