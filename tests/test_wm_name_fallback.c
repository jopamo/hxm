...
void test_net_wm_icon_name_fallback_enqueue_failure_finalizes_manage(void) {
  server_t s;
  memset(&s, 0, sizeof(s));
  s.is_test = true;
  s.conn = xcb_connect(NULL, NULL);

  atoms._NET_WM_ICON_NAME = 30;
  atoms.WM_ICON_NAME = 31;
  atoms.UTF8_STRING = 32;
  ...
  printf("test_net_wm_icon_name_fallback_enqueue_failure_finalizes_manage passed\n");
...
int main(void) {
  ...
  test_net_wm_icon_name_fallback_enqueue_failure_finalizes_manage();
  return 0;
}
