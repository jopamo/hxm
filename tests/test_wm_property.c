...
void test_property_dirty_bits(void) {
  ...
  // Mock flush (can't really test cookie push without mock connection, but we
  // can verify logic flow if we had one) For unit test, verifying bits are set
...
void test_property_net_wm_desktop_enqueues_cookie(void) {
  ...
  // Avoid atoms_init here to prevent atom id collisions with early-return
  // filters.
...
void test_property_net_wm_state_enqueues_cookie(void) {
  server_t s;
  memset(&s, 0, sizeof(s));
  s.is_test = true;
  ...
  printf("test_property_net_wm_state_enqueues_cookie passed\n");
...
void test_property_net_wm_window_type_enqueues_cookie(void) {
  server_t s;
  memset(&s, 0, sizeof(s));
  s.is_test = true;
  ...
  memset(&ev, 0, sizeof(ev));
  ev.window = hot->xid;
  ev.atom = atoms._NET_WM_WINDOW_TYPE;
  ...
  printf("test_property_net_wm_window_type_enqueues_cookie passed\n");
...
int main(void) {
  ...
  test_property_net_wm_state_enqueues_cookie();
  test_property_net_wm_window_type_enqueues_cookie();
  return 0;
}
