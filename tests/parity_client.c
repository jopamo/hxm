        ...
        if (ev) {
            if ((ev->response_type & ~0x80) == XCB_REPARENT_NOTIFY) {
                ...
                if (re->window == w && re->parent != root) {
                    if (out_frame) *out_frame = re->parent;
                    free(ev);
                    return;
                }
            }
...
static void wait_root_cardinal_eq(xcb_atom_t prop, uint32_t want, uint32_t timeout_ms) {
    ...
    while (now_us() < deadline) {
        uint32_t v = 0;
        if (get_cardinal32(root, prop, &v) && v == want) return;
        drain_events_some();
        usleep(2000);
    }
    uint32_t got = 0;
    if (!get_cardinal32(root, prop, &got)) fail("root cardinal missing during wait");
...
static void wait_window_cardinal_eq(xcb_window_t w, xcb_atom_t prop, uint32_t want, uint32_t timeout_ms) {
    ...
    while (now_us() < deadline) {
        uint32_t v = 0;
        if (get_cardinal32(w, prop, &v) && v == want) return;
        drain_events_some();
        usleep(2000);
    }
    uint32_t got = 0;
    if (!get_cardinal32(w, prop, &got)) fail("window cardinal missing during wait");
...
static void wait_window_destroyed_or_unlisted(xcb_window_t w, uint32_t timeout_ms) {
    ...
    while (now_us() < deadline) {
        ...
        // property path
        if (!window_list_contains(atoms._NET_CLIENT_LIST, w)) return;
...
static xcb_window_t create_window(const char* class_name, const char* instance_name) {
    ...
    xcb_create_window(c, XCB_COPY_FROM_PARENT, w, root, 0, 0, 100, 100, 0, XCB_WINDOW_CLASS_INPUT_OUTPUT,
...
static void test_wm_sanity(void) {
    ...
    if (sup2 != sup) fail("_NET_SUPPORTING_WM_CHECK not self-referential on supporting window");
...
static void test_client_lists_and_manage(void) {
    ...
    if (!window_list_contains(atoms._NET_CLIENT_LIST, w)) fail("_NET_CLIENT_LIST does not contain window");
...
static void test_active_window_focus(void) {
    ...
    while (now_us() < deadline) {
        ...
        if (get_window_prop(root, atoms._NET_ACTIVE_WINDOW, &aw) && aw == w1) break;
        drain_events_some();
        usleep(2000);
    }
    xcb_window_t aw = XCB_WINDOW_NONE;
    if (!get_window_prop(root, atoms._NET_ACTIVE_WINDOW, &aw) || aw != w1) fail("_NET_ACTIVE_WINDOW did not become w1");
    ...
    while (now_us() < deadline) {
        ...
        if (get_window_prop(root, atoms._NET_ACTIVE_WINDOW, &cur) && cur == w2) break;
        drain_events_some();
        usleep(2000);
    }
    if (!get_window_prop(root, atoms._NET_ACTIVE_WINDOW, &aw) || aw != w2) fail("_NET_ACTIVE_WINDOW did not become w2");
...
static void test_workspaces(void) {
    ...
    if (!get_cardinal32(root, atoms._NET_CURRENT_DESKTOP, &cur)) fail("_NET_CURRENT_DESKTOP missing");

    if (cur != 0) fail("Initial desktop not 0");

    uint32_t nd = 0;
    if (get_cardinal32(root, atoms._NET_NUMBER_OF_DESKTOPS, &nd)) {
        if (nd == 0) fail("_NET_NUMBER_OF_DESKTOPS is zero");
        if (nd < 2) fprintf(stderr, "WARN: only %u desktops\n", nd);
    } else {
    ...
    // by default windows should be on current desktop or on 0 depending on your policy
    // we check that _NET_WM_DESKTOP exists and is sane
    uint32_t wdesk = 0xffffffffu;
    if (!get_cardinal32(w, atoms._NET_WM_DESKTOP, &wdesk)) fail("Window missing _NET_WM_DESKTOP");
    ...
    if (!get_cardinal32(w, atoms._NET_WM_DESKTOP, &wdesk2)) fail("Window missing _NET_WM_DESKTOP after switch");
    if (wdesk2 != wdesk) {
...
static void test_fullscreen_state_and_geometry(void) {
    ...
    while (now_us() < deadline) {
        if (atom_list_contains(w, atoms._NET_WM_STATE, atoms._NET_WM_STATE_FULLSCREEN)) break;
        drain_events_some();
        usleep(2000);
    }
    ...
    if (!gr) fail("Failed to get geometry");
...
static void test_state_above_below_sticky(void) {
    ...
    uint64_t deadline = now_us() + 1000ull * 1000ull;
    while (now_us() < deadline) {
        if (atom_list_contains(w, atoms._NET_WM_STATE, atoms._NET_WM_STATE_ABOVE)) break;
        drain_events_some();
        usleep(2000);
    }
    ...
    deadline = now_us() + 1000ull * 1000ull;
    while (now_us() < deadline) {
        if (atom_list_contains(w, atoms._NET_WM_STATE, atoms._NET_WM_STATE_BELOW)) break;
        drain_events_some();
        usleep(2000);
    }
    ...
    deadline = now_us() + 1000ull * 1000ull;
    while (now_us() < deadline) {
        if (atom_list_contains(w, atoms._NET_WM_STATE, atoms._NET_WM_STATE_STICKY)) break;
        drain_events_some();
        usleep(2000);
    }
...
static void test_rules_probe(void) {
    ...
    if (!get_cardinal32(w, atoms._NET_WM_DESKTOP, &d)) fail("rules probe window missing _NET_WM_DESKTOP");
