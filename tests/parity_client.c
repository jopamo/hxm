...
static bool get_cardinal32(xcb_window_t win, xcb_atom_t prop, uint32_t *out) {
  uint32_t nbytes = 0;
  uint32_t *v =
      (uint32_t *)get_property_any(win, prop, XCB_ATOM_CARDINAL, &nbytes);
  if (!v || nbytes < 4) {
    free(v);
    return false;
  }
...
static bool get_window_prop(xcb_window_t win, xcb_atom_t prop,
                            xcb_window_t *out) {
  uint32_t nbytes = 0;
  xcb_window_t *v =
      (xcb_window_t *)get_property_any(win, prop, XCB_ATOM_WINDOW, &nbytes);
...
static bool atom_list_contains(xcb_window_t win, xcb_atom_t prop,
                               xcb_atom_t needle) {
  uint32_t nbytes = 0;
  xcb_atom_t *v =
      (xcb_atom_t *)get_property_any(win, prop, XCB_ATOM_ATOM, &nbytes);
...
static void wait_managed(xcb_window_t w, uint32_t timeout_ms,
                         xcb_window_t *out_frame) {
...
static void wait_root_cardinal_eq(xcb_atom_t prop, uint32_t want,
                                  uint32_t timeout_ms) {
  uint64_t deadline = now_us() + (uint64_t)timeout_ms * 1000ull;
  while (now_us() < deadline) {
    uint32_t v = 0;
...
static void wait_window_cardinal_eq(xcb_window_t w, xcb_atom_t prop,
...
static void wait_window_destroyed_or_unlisted(xcb_window_t w,
                                              uint32_t timeout_ms) {
  uint64_t deadline = now_us() + (uint64_t)timeout_ms * 1000ull;
  while (now_us() < deadline) {
    // event path
...
static xcb_window_t create_window(const char *class_name,
  ...
  xcb_create_window(c, XCB_COPY_FROM_PARENT, w, root, 0, 0, 100, 100, 0,
  ...
  if (class_name || instance_name) {
    ...
    xcb_change_property(c, XCB_PROP_MODE_REPLACE, w, atoms.WM_CLASS,
                        XCB_ATOM_STRING, 8, (uint32_t)len, buf);
    free(buf);
  }

  // set WM_DELETE_WINDOW so the WM can choose to send it
  xcb_change_property(c, XCB_PROP_MODE_REPLACE, w, atoms.WM_PROTOCOLS,
                      XCB_ATOM_ATOM, 32, 1, &atoms.WM_DELETE_WINDOW);
...
static void send_wm_state_request(xcb_window_t w, uint32_t action,
                                  xcb_atom_t a1, xcb_atom_t a2) {
  xcb_client_message_event_t ev = {0};
  ev.response_type = XCB_CLIENT_MESSAGE;
  ev.format = 32;
  ev.window = w;
...
static void test_wm_sanity(void) {
  ...
  if (!get_window_prop(root, atoms._NET_SUPPORTING_WM_CHECK, &sup) ||
      sup == XCB_WINDOW_NONE)
  ...
  if (!atom_list_contains(root, atoms._NET_SUPPORTED,
...
static void test_workspaces(void) {
  ...
  if (wdesk2 != wdesk) {
    fprintf(stderr,
            "WARN: window desktop changed from %u to %u on desktop switch\n",
            wdesk, wdesk2);
  }
...
static void test_fullscreen_state_and_geometry(void) {
  ...
  while (now_us() < deadline) {
    if (atom_list_contains(w, atoms._NET_WM_STATE,
