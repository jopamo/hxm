...
static int poll_badaccess_once(xcb_connection_t *c, unsigned int request,
                               void **reply, xcb_generic_error_t **error) {
  (void)c;
  (void)request;

  if (!g_force_badaccess_once)
    return 0;
...
static void test_existing_wm_keeps_selection_owner(void) {
  ...
  const struct stub_prop_call *root_call =
      find_prop_call(s1.root, atoms._NET_SUPPORTING_WM_CHECK);
...
static void test_wm_s0_selection_and_supporting_check(void) {
  ...
  const struct stub_prop_call *root_call =
      find_prop_call(s.root, atoms._NET_SUPPORTING_WM_CHECK);
  assert(root_call != NULL);
  ...
  const struct stub_prop_call *support_call =
      find_prop_call(s.supporting_wm_check, atoms._NET_SUPPORTING_WM_CHECK);
