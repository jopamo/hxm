...
void test_stale_reply_dropped(void) {
    ...
    cookie_slot_t stale_slot = {
        .sequence = 1, .type = COOKIE_GET_PROPERTY, .client = h, .txn_id = 5, .handler = mock_handler, .live = true};
    ...
    cookie_slot_t fresh_slot = {
        .sequence = 2, .type = COOKIE_GET_PROPERTY, .client = h, .txn_id = 15, .handler = mock_handler, .live = true};
