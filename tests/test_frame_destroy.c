...
static xcb_generic_event_t make_destroy_event(uint8_t response_type,
                                              xcb_window_t win,
...
static xcb_generic_event_t make_unmap_event(xcb_window_t win,
                                            xcb_window_t event_win) {
...
static handle_t test_create_managed_client(server_t *s, xcb_window_t client_xid,
                                           xcb_window_t frame_xid) {
  ...
  hot->state =
      STATE_MAPPED; // Use STATE_MAPPED which maps to a mapped state usually
  hot->xid = client_xid;
  hot->frame = frame_xid;
  ...
  // If your window_to_client also tracks frames (some WMs do, some don't,
  // usually frame_to_client is separate) Based on test_resize.c, there is
  // s->frame_to_client.
...
static void test_frame_destroy_during_interaction_cancels_only(void) {
  ...
  wm_start_interaction(&s, h, c, /*start_move*/ false, /*resize_dir*/ 9,
                       /*root_x*/ 1919, /*root_y*/ 0, 0, false);
  ...
  xcb_generic_event_t ge =
      make_destroy_event(XCB_DESTROY_NOTIFY, frame_xid, s.root);

  // We need to call the specific handler because event_process_one might need
  // more setup or is static wm_handle_destroy_notify is in wm.h
...
static void test_unmanage_cancels_interaction_before_frame_destroy(void) {
  ...
  // Checking client.h or similar. Based on list of files, src/client.c likely
  // has it. include/client.h likely declares it. Let's assume
  // client_unmanage(server_t* s, handle_t h) based on naming convention or
  // maybe it's just triggered by UnmapNotify/DestroyNotify on the client
  // window.
  ...
  // Looking at include/wm.h, there isn't an explicit unmanage function exposed
  // there, but wm_handle_unmap_notify calls it.

  // Let's look at src/client.c or include/client.h to find the unmanage
  // function. For now I will assume `client_unmanage` exists and is accessible.
