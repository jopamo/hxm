...
static xcb_get_property_reply_t *make_string_reply(xcb_atom_t type,
...
static void test_net_wm_name_update(void) {
  ...
  xcb_get_property_reply_t *rep =
      make_string_reply(atoms.UTF8_STRING, "NetTitle", 8);
...
static void test_wm_name_fallback(void) {
  ...
  xcb_get_property_reply_t *rep_legacy =
      make_string_reply(XCB_ATOM_STRING, "LegacyTitle", 11);
...
static void test_title_truncation(void) {
  ...
  xcb_get_property_reply_t *rep =
      make_string_reply(atoms.UTF8_STRING, long_title, 4999);
  wm_handle_reply(&s, &slot, rep, NULL);
  free(rep);

  assert(cold->title != NULL);
  // MAX_TITLE_BYTES is 4096 in wm_reply.c, but since it's internal we can't
  // check the constant directly easily unless it's in a header. We check the
  // length is 4096.
