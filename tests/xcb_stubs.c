...
int (*stub_poll_for_reply_hook)(xcb_connection_t *c, unsigned int request,
                                void **reply,
...
xcb_visualtype_t *xcb_get_visualtype(xcb_connection_t *conn,
...
const stub_config_call_t *stub_config_call_at(int idx) {
  if (idx < 0 || idx >= stub_config_calls_len)
    return NULL;
...
xcb_void_cookie_t xcb_change_property(xcb_connection_t *c, uint8_t mode,
  ...
  if (byte_len > STUB_MAX_PROP_BYTES)
    byte_len = STUB_MAX_PROP_BYTES;
  if (byte_len && data)
...
xcb_get_window_attributes(xcb_connection_t *c, xcb_window_t window) {
  ...
  if (stub_attr_requests_len < STUB_MAX_ATTR_REQUESTS) {
    stub_attr_requests[stub_attr_requests_len++] =
        (stub_attr_request_t){cookie.sequence, window};
  }
  return cookie;
}
...
void xcb_stubs_set_query_tree_children(const xcb_window_t *children, int len) {
  if (len < 0)
    len = 0;
  if (len > (int)(sizeof(stub_query_tree_children) /
                  sizeof(stub_query_tree_children[0]))) {
    len = (int)(sizeof(stub_query_tree_children) /
                sizeof(stub_query_tree_children[0]));
  }
  memset(stub_query_tree_children, 0, sizeof(stub_query_tree_children));
  for (int i = 0; i < len; i++) {
...
bool xcb_stubs_attr_request_window(uint32_t seq, xcb_window_t *out_window) {
  for (int i = 0; i < stub_attr_requests_len; i++) {
    if (stub_attr_requests[i].seq == seq) {
      if (out_window)
        *out_window = stub_attr_requests[i].window;
      return true;
    }
  }
  return false;
...
xcb_void_cookie_t xcb_send_event(xcb_connection_t *c, uint8_t propagate,
  ...
  if (event)
    memcpy(stub_last_event, event, sizeof(stub_last_event));
  return (xcb_void_cookie_t){0};
}
...
void xcb_stubs_set_selection_owner(xcb_window_t owner) {
  stub_selection_owner = owner;
}

xcb_window_t xcb_stubs_get_selection_owner(void) {
  return stub_selection_owner;
}

// Event queue helpers
static bool stub_events_push(xcb_generic_event_t **buf, size_t *head,
                             size_t *len, xcb_generic_event_t *ev) {
  if (*len >= STUB_MAX_EVENTS)
    return false;
...
static xcb_generic_event_t *stub_events_pop(xcb_generic_event_t **buf,
...
bool xcb_stubs_enqueue_queued_event(xcb_generic_event_t *ev) {
  return stub_events_push(stub_queued_events, &stub_queued_head,
                          &stub_queued_len, ev);
}
...
xcb_generic_event_t *xcb_poll_for_queued_event(xcb_connection_t *c) {
  (void)c;
  return stub_events_pop(stub_queued_events, &stub_queued_head,
                         &stub_queued_len);
}
...
xcb_void_cookie_t xcb_put_image(xcb_connection_t *c, uint8_t format,
  ...
  if (data && data_len > 0) {
    ...
    if (copy_len > sizeof(stub_last_image_data))
      copy_len = sizeof(stub_last_image_data);
...
xcb_randr_get_screen_resources_current_reply(
    xcb_connection_t *c, xcb_randr_get_screen_resources_current_cookie_t cookie,
    xcb_generic_error_t **e) {
  (void)c;
  (void)cookie;
  (void)e;
...
xcb_void_cookie_t xcb_randr_select_input(xcb_connection_t *c,
                                         xcb_window_t window, uint16_t enable) {
  (void)c;
  (void)window;
  (void)enable;
  return (xcb_void_cookie_t){0};
...
xcb_randr_query_version(xcb_connection_t *c, uint32_t major_version,
