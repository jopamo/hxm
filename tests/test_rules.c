...
void test_rules_matching(void) {
  ...
  // Wait, I can make it public or just test via client_finish_manage with
  // stubs.
  ...
  // Center placement: (1920-400)/2 = 760, (1080-300-20-4)/2 -> wait,
  // title_height=20, border=2 geom.w + 2*border, geom.h + title + border But
  // hot->desired is just the client geom. wm_place_window uses hot->desired.
