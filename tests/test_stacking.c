...
static void assert_layer_order(const server_t *s, int layer,
                               const handle_t *handles, size_t count) {
...
void test_stack_raise_transients_restack_count(void) {
  ...
  t1->transient_for = hp;
  list_insert(&t1->transient_sibling, p->transients_head.prev,
              &p->transients_head);
  t2->transient_for = hp;
  list_insert(&t2->transient_sibling, p->transients_head.prev,
              &p->transients_head);
