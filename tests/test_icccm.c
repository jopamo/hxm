...
void test_client_close(void) {
  ...
  xcb_client_message_event_t *ev =
      (xcb_client_message_event_t *)stub_last_event;
...
void test_wm_state_manage_unmanage(void) {
  ...
  const struct stub_prop_call *set =
      find_prop_call(hot->xid, atoms.WM_STATE, false);
  assert(set != NULL);
  ...
  const struct stub_prop_call *del =
      find_prop_call(hot->xid, atoms.WM_STATE, true);
  assert(del != NULL);
