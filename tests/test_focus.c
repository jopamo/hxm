...
void test_should_focus_on_map(void) {
  ...
  const window_type_t no_focus_types[] = {
      WINDOW_TYPE_DOCK, WINDOW_TYPE_NOTIFICATION, WINDOW_TYPE_DESKTOP, WINDOW_TYPE_MENU, WINDOW_TYPE_DROPDOWN_MENU, WINDOW_TYPE_POPUP_MENU, WINDOW_TYPE_TOOLTIP, WINDOW_TYPE_COMBO, WINDOW_TYPE_DND,
  };
...
void test_debug_dump_focus_history_guard(void) {
  ...
  // wm_set_focus calls debug_dump_focus_history internally if debug logging is
  // enabled.
  for (int i = 0; i < CLIENT_COUNT; i++) {
  ...
  printf(
      "test_debug_dump_focus_history_guard passed (check logs for WARN if "
      "needed)\n");
  teardown_server(&s);
}
