...
extern int (*stub_poll_for_reply_hook)(xcb_connection_t *c,
...
static void mock_handler(struct server *s, const struct cookie_slot *slot,
                         void *reply, xcb_generic_error_t *err) {
  (void)s;
...
static int mock_poll(xcb_connection_t *c, unsigned int request, void **reply,
                     xcb_generic_error_t **error) {
  (void)c;
...
static void require_drain_until_called(cookie_jar_t *cj, int max_iters,
                                       int max_replies_per_iter) {
  int it = 0;
  ...
  if (!g_handler_called)
    fail("handler not called within drain loop");
}
...
static void test_push_and_drain(void) {
  ...
  uint32_t seq = 123;
  bool pushed = cookie_jar_push(&cj, seq, COOKIE_GET_GEOMETRY, HANDLE_INVALID,
                                0, 0, mock_handler);
  assert(pushed);
...
static void test_duplicate_push_rejected_or_replaced(void) {
  ...
  uint32_t seq = 42;
  bool p1 = cookie_jar_push(&cj, seq, COOKIE_GET_GEOMETRY, HANDLE_INVALID, 0, 0,
                            mock_handler);
  assert(p1);
  assert(cj.live_count == 1);

  // Depending on implementation, pushing same seq may be rejected or replace
  bool p2 = cookie_jar_push(&cj, seq, COOKIE_GET_GEOMETRY, HANDLE_INVALID, 0, 0,
                            mock_handler);
...
static void test_drain_budget_respected(void) {
  ...
  for (uint32_t i = 1; i <= N; i++) {
    bool ok = cookie_jar_push(&cj, i, COOKIE_GET_GEOMETRY, HANDLE_INVALID, 0, 0,
                              mock_handler);
    assert(ok);
  }
...
static void test_growth_and_reachability(void) {
  ...
  for (uint32_t i = 1; i <= N; i++) {
    bool pushed = cookie_jar_push(&cj, i, COOKIE_GET_GEOMETRY, HANDLE_INVALID,
                                  0, 0, mock_handler);
    assert(pushed);
  }
...
static void test_collisions_linear_probe(void) {
  ...
  // If init starts larger, this still collides for many masks, but this is the
  // simplest. We only rely on the fact your index uses low bits with a
  // power-of-2 mask. If you change hashing, this test will still pass because
  // it doesn't require a collision, it just requires both entries to be
  // drainable.
  stub_poll_for_reply_hook = mock_poll;

  bool p1 = cookie_jar_push(&cj, 1, COOKIE_GET_GEOMETRY, HANDLE_INVALID, 0, 0,
                            mock_handler);
  bool p2 = cookie_jar_push(&cj, 17, COOKIE_GET_GEOMETRY, HANDLE_INVALID, 0, 0,
                            mock_handler);
  assert(p1 && p2);
...
static void test_remove_breaks_chain_regression(void) {
  ...
  // deleting an element in a probe chain without backshift/rehash breaks lookup
  // for later elements. We force a cluster by using many sequential seq values.
  stub_poll_for_reply_hook = mock_poll;
  ...
  for (uint32_t i = 0; i < count; i++) {
    bool ok = cookie_jar_push(&cj, base + i, COOKIE_GET_GEOMETRY,
                              HANDLE_INVALID, 0, 0, mock_handler);
    assert(ok);
  }
...
static void test_error_path(void) {
  ...
  reset_handler_state();
  bool pushed = cookie_jar_push(&cj, 500, COOKIE_GET_GEOMETRY, HANDLE_INVALID,
                                0, 0, mock_handler);
  assert(pushed);
...
static void test_reply_and_error_both(void) {
  ...
  reset_handler_state();
  bool pushed = cookie_jar_push(&cj, 501, COOKIE_GET_GEOMETRY, HANDLE_INVALID,
                                0, 0, mock_handler);
  assert(pushed);
...
static void test_timeout(void) {
  ...
  bool pushed = cookie_jar_push(&cj, 999, COOKIE_GET_GEOMETRY, HANDLE_INVALID,
                                0, 0, mock_handler);
  assert(pushed);
...
static void test_timeout_then_late_reply_ignored(void) {
  ...
  reset_handler_state();
  bool pushed = cookie_jar_push(&cj, 1001, COOKIE_GET_GEOMETRY, HANDLE_INVALID,
                                0, 0, mock_handler);
  assert(pushed);
  ...
  // now pretend reply arrives late; jar must not resurrect or call handler
  // again
  reset_handler_state();
...
static void test_cursor_fairness_progress(void) {
  ...
  for (size_t i = 0; i < n; i++) {
    bool ok = cookie_jar_push(&cj, keys[i], COOKIE_GET_GEOMETRY, HANDLE_INVALID,
                              0, 0, mock_handler);
    assert(ok);
  }
  assert(cj.live_count == n);

  // Mark only one key as ready; drain with small max_replies so we force cursor
  // to move If cursor never advances on non-ready slots, this can starve items
...
static void test_performance_smoke(void) {
  ...
  for (int i = 0; i < N; i++) {
    bool ok = cookie_jar_push(&cj, (uint32_t)(i + 1), COOKIE_GET_GEOMETRY,
                              HANDLE_INVALID, 0, 0, mock_handler);
    assert(ok);
  }
  ...
  uint64_t start =
      (uint64_t)ts_start.tv_sec * 1000000000ULL + (uint64_t)ts_start.tv_nsec;
  uint64_t mid =
      (uint64_t)ts_mid.tv_sec * 1000000000ULL + (uint64_t)ts_mid.tv_nsec;
  uint64_t end =
      (uint64_t)ts_end.tv_sec * 1000000000ULL + (uint64_t)ts_end.tv_nsec;

  printf("Performance: inserted %d in %" PRIu64 " ns (%.2f ns/item)\n", N,
         (mid - start), (double)(mid - start) / (double)N);
...
static void test_alloc_fail_grow(void) {
  ...
  if (pid == 0) {
    ...
    for (int i = 0; i < 2000; i++) {
      cookie_jar_push(&cj, i, COOKIE_GET_GEOMETRY, HANDLE_INVALID, 0, 0,
                      mock_handler);
    }
