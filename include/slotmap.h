 ...
 *
 * A container that provides stable, non-dangling references (handles) to
 * objects stored in an arena, even when objects are deleted and indices are
 * reused
 *
 * Core idea:
...
static inline uint32_t slotmap_capacity(const slotmap_t *sm) {
  return sm ? sm->cap : 0u;
}
...
static inline bool slotmap_init_ex(slotmap_t *sm, uint32_t cap, size_t hot_sz,
                                   size_t cold_sz, slotmap_allocator_t a) {
  if (!sm)
    return false;

  slotmap__reset(sm);

  if (!a.malloc_fn || !a.calloc_fn || !a.free_fn)
    return false;
  ...
  sm->hdr =
      (slot_hdr_t *)sm->a.calloc_fn(sm->a.ctx, (size_t)cap, sizeof(slot_hdr_t));
  if (!sm->hdr) {
    slotmap__reset(sm);
    return false;
  }
...
static inline bool slotmap_init(slotmap_t *sm, uint32_t cap, size_t hot_sz,
                                size_t cold_sz) {
...
static inline void *slotmap_hot(const slotmap_t *sm, handle_t h) {
  return slotmap_hot_unchecked(sm, handle_index(h));
}
 ...
 * Callback must not call slotmap_alloc/slotmap_free/slotmap_reserve on the same
 * slotmap
 */
static inline void slotmap_for_each_used(slotmap_t *sm, slotmap_visit_fn fn,
                                         void *user) {
  if (!sm || !sm->hdr || !fn)
    return;
 ...
 * Allocates fresh buffers and copies existing content so failure leaves sm
 * unchanged
 */
static inline bool slotmap_reserve(slotmap_t *sm, uint32_t new_cap) {
  ...
  if (!a.calloc_fn || !a.malloc_fn || !a.free_fn)
    return false;

  size_t hdr_bytes = 0;
  if (slotmap__mul_overflow_size((size_t)new_cap, sizeof(slot_hdr_t),
                                 &hdr_bytes))
    return false;
...
static inline bool slotmap_is_full(const slotmap_t *sm) {
  return !sm || sm->free_head == 0;
}

static inline handle_t slotmap_alloc(slotmap_t *sm, void **out_hot,
                                     void **out_cold) {
  if (!sm || !sm->hdr)
    return HANDLE_INVALID;
 ...
 */
static inline handle_t slotmap_alloc_grow(slotmap_t *sm, void **out_hot,
                                          void **out_cold) {
 ...
 */
static inline void slotmap_clear_with(slotmap_t *sm, slotmap_visit_fn dtor,
                                      void *user) {
  if (!sm || !sm->hdr)
    return;
  if (dtor)
    slotmap_for_each_used(sm, dtor, user);
  slotmap_clear(sm);
}
