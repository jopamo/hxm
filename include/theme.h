 ...
 * Provides appearance/background style flags and color values used by render/frame/menu code
 *
 * Conventions:
...
static inline bool theme_style_is_gradient(background_style_t flags) { return theme_style_has(flags, BG_GRADIENT); }

/* Color channel extraction for 0xAARRGGBB */
static inline uint8_t theme_color_a(uint32_t argb) { return (uint8_t)((argb >> 24) & 0xFFu); }
static inline uint8_t theme_color_r(uint32_t argb) { return (uint8_t)((argb >> 16) & 0xFFu); }
static inline uint8_t theme_color_g(uint32_t argb) { return (uint8_t)((argb >> 8) & 0xFFu); }
static inline uint8_t theme_color_b(uint32_t argb) { return (uint8_t)((argb >> 0) & 0xFFu); }

/* Convert 0xAARRGGBB to premultiplied or straight doubles (0..1)
 * These are straight (non-premultiplied) components, suitable for cairo_set_source_rgba
 */
static inline double theme_color_a_f(uint32_t argb) { return (double)theme_color_a(argb) / 255.0; }
static inline double theme_color_r_f(uint32_t argb) { return (double)theme_color_r(argb) / 255.0; }
static inline double theme_color_g_f(uint32_t argb) { return (double)theme_color_g(argb) / 255.0; }
static inline double theme_color_b_f(uint32_t argb) { return (double)theme_color_b(argb) / 255.0; }
...
static inline bool theme_style_orientation_valid(background_style_t flags) {
    uint32_t o = (uint32_t)flags &
                 ((uint32_t)BG_VERTICAL | (uint32_t)BG_HORIZONTAL | (uint32_t)BG_DIAGONAL | (uint32_t)BG_CROSSDIAGONAL);
    return (o == 0u) || (o == (uint32_t)BG_VERTICAL) || (o == (uint32_t)BG_HORIZONTAL) ||
           (o == (uint32_t)BG_DIAGONAL) || (o == (uint32_t)BG_CROSSDIAGONAL);
}
