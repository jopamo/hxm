...
static inline bool theme_style_has(background_style_t flags,
                                   background_style_t bit) {
...
static inline bool theme_style_orientation_valid(background_style_t flags) {
  uint32_t o =
      (uint32_t)flags & ((uint32_t)BG_VERTICAL | (uint32_t)BG_HORIZONTAL |
                         (uint32_t)BG_DIAGONAL | (uint32_t)BG_CROSSDIAGONAL);
  return (o == 0u) || (o == (uint32_t)BG_VERTICAL) ||
         (o == (uint32_t)BG_HORIZONTAL) || (o == (uint32_t)BG_DIAGONAL) ||
         (o == (uint32_t)BG_CROSSDIAGONAL);
}
