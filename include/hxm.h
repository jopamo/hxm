 ...
 * - Bounded tick: each loop iteration processes at most a bounded amount of work
 * - Coalesce by default
 * - Batch X requests
 */
...
static inline bool list_empty(const list_node_t* head) { return head->next == head; }
...
static inline void list_push_back(list_node_t* head, list_node_t* node) { list_insert_between(node, head->prev, head); }
...
static inline bool list_is_linked(const list_node_t* node) { return node->next != node; }
